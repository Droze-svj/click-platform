...
async function initializeAllWorkers() {
  ...
  try {
    ...
    // This prevents BullMQ from defaulting to localhost and spamming ECONNREFUSED
    const { getRedisConnection, verifyRedisConnection } = require('../services/jobQueueService');
    ...
    // Pre-flight check: Ping Redis to ensure it's actually reachable
    logger.info('🛰️ Performing Redis pre-flight connectivity check...');
    const isRedisReachable = await verifyRedisConnection(redisConnection);

    if (!isRedisReachable) {
      const warningMsg = '⚠️ Redis pre-flight check failed (Connectivity Refused or Not Found).';
      logger.warn(warningMsg);
      logger.warn('⚠️ Workers will NOT be initialized to prevent infinite log spam.');
      logger.warn('⚠️ Ensure Redis is running and REDIS_URL/REDIS_HOST is correct to enable background jobs.');
      return;
    }

    logger.info('🚀 Redis connectivity verified. Proceding with worker creation...');
    ...
    const { initializeTokenRefreshWorker } = require('./tokenRefreshWorker');
    ...
    const refreshWorker = initializeTokenRefreshWorker();

    // Check if any workers were actually created
    const workersCreated = [videoWorker, contentWorker, emailWorker, transcriptWorker, socialWorker, refreshWorker].filter(w => w !== null).length;
...
module.exports = {
  ...
  shutdownAllWorkers,
  QUEUE_PROCESSORS: {
    'content-generation': require('./contentGenerator').processContentJob,
    'video-processing': require('./videoProcessor').processVideoJob,
    'email-sending': require('./emailSender').processEmailJob,
    'transcript-generation': require('./transcriptProcessor').processTranscriptJob,
    'social-posting': require('./socialPostProcessor').processSocialPostJob,
    'token-refresh': require('./tokenRefreshWorker').processTokenRefreshJob,
  }
};
