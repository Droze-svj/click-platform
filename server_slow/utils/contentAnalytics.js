...
const advancedEvergreenService = require('../services/advancedEvergreenService');
...
async function getContentInsights(userId) {
  ...
  try {
    ...
    if (evergreenCandidates && evergreenCandidates.length > 0) {
      ...
      insights.opportunities.push({
        type: 'recycle',
        message: `Phase 17 analysis suggests recycling your top evergreen piece to capture 2026 long-tail momentum.`,
        action: 'View in Vault'
      });
    }
  } catch (evergreenErr) {
    logger.warn('Evergreen detection during insights failed:', evergreenErr.message);
  }
