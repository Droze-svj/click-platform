...
/**
 * Scout for Resurrection Candidates (Phase 19)
 * Identifies "Golden Records" with legacy momentum.
 */
async function scoutForResurrectionCandidates(userId) {
  try {
    const MIN_AGE_DAYS = 90;
    const cutoffDate = new Date();
    cutoffDate.setDate(cutoffDate.getDate() - MIN_AGE_DAYS);

    logger.info('Phase 19: Scouting for Golden Records...', { userId, cutoffDate });

    // Find pieces that were high performers (>10k views or >5% engagement) 
    // and are at least 3 months old.
    const candidates = await ScheduledPost.find({
      userId,
      status: 'posted',
      postedAt: { $lte: cutoffDate },
      $or: [
        { 'analytics.views': { $gte: 10000 } },
    ...
    return candidates.map(c => ({
      originalPostId: c._id,
      contentId: c.content?.contentId?._id || c.contentId,
      title: c.content?.contentId?.title || 'Legacy Asset',
      historicalViews: c.analytics?.views || 0,
      historicalEngagement: c.analytics?.engagement || 0,
      ageInDays: Math.floor((new Date() - new Date(c.postedAt)) / (1000 * 60 * 60 * 24)),
      signalStrength: calculateResurrectionSignal(c)
    }));
  } catch (error) {
    logger.error('Resurrection Scouting failed', { error: error.message, userId });
    throw error;
  }
}

function calculateResurrectionSignal(post) {
  const views = post.analytics?.views || 0;
  const engagement = post.analytics?.engagement || 0;
  const signal = Math.min(100, (views / 200) + (engagement / 10));
...
module.exports = {
  ...
  createRepostABTest,
  scoutForResurrectionCandidates
};
