...
async function trackAction(userId, action, metadata = {}) {
  try {
    ...
    // Analyze patterns and suggest workflows (backgrounded to improve performance)
    setImmediate(() => {
      analyzePatterns(userId).catch(err => {
        logger.error('Background pattern analysis failed', { error: err.message, userId });
      });
    });
