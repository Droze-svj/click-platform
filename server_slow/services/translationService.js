...
/**
 * Delta Translation: Translates specific segments while maintaining context
 */
async function translateSegments(contentId, language, segmentsToTranslate, contextSegments = []) {
  try {
    const targetLangFull = SUPPORTED_LANGUAGES[language.toLowerCase()] || language;
    
    let prompt = `You are a professional translator. Translate the following "Target Segments" to ${targetLangFull}.\n`;
    prompt += `Maintain consistency with the "Context Segments" (already translated).\n\n`;
    
    if (contextSegments.length > 0) {
      prompt += `### Context Segments:\n`;
      contextSegments.forEach(s => {
        prompt += `Original: ${s.originalText}\nTranslated: ${s.translatedText}\n---\n`;
      });
    }
    
    prompt += `\n### Target Segments to Translate:\n`;
    segmentsToTranslate.forEach((s, idx) => {
      prompt += `ID: ${idx}\nText: ${s.originalText}\n---\n`;
    });
    
    prompt += `\nRespond with a JSON array of objects: [{ idx: number, translatedText: string }]`;

    const systemMsg = `Accurate, natural translation for ${targetLangFull}. Only return the JSON requested.`;
    const fullPrompt = `${systemMsg}\n\n${prompt}`;
