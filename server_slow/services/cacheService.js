...
async function initCache() {
  ...
  // Skip Redis if not configured, or if localhost/placeholder
  const isLocalhost = redisUrl?.includes('localhost') || redisUrl?.includes('127.0.0.1') || redisUrl?.includes('::1');
  const isDev = process.env.NODE_ENV !== 'production';

  if (!redisUrl || redisUrl.includes('placeholder') || (isDev && isLocalhost)) {
    if (redisUrl?.includes('placeholder')) {
      logger.info('Redis URL is placeholder. Caching disabled.');
    } else if (isDev && isLocalhost) {
      logger.info('🔧 [Redis Guard] Skipping localhost connection in development.');
    } else {
