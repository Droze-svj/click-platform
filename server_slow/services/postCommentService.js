...
async function addComment(postId, userId, commentData) {
  try {
    ...
    // 📡 Phase 20: Real-Time Community Pulse Trigger
    try {
      const communityAgentService = require('./communityAgentService');
      // Throttled trigger for re-calculating pulse
      communityAgentService.getStatus(userId); 
    } catch (err) {
      logger.warn('CM Agent Pulse trigger failed', { error: err.message });
    }
