...
// In development, if we fail to init cloud storage or if we want to force local, do it here
const isDev = process.env.NODE_ENV !== 'production';
const forceLocalInDev = process.env.FORCE_LOCAL_STORAGE === 'true' || (isDev && !process.env.FORCE_CLOUD_IN_DEV);

if (forceLocalInDev && useCloudStorage) {
  logger.info('🔧 [Storage] Dev mode detected: Forcing local storage fallback despite cloud config.');
...
if (!useCloudStorage) {
  ...
  logger.warn('⚠️ Cloud storage not active. Using local file storage.', { envStatus });
}
 ...
 * Useful for transitioning from local dev to production
 */
async function migrateLocalToCloud(callback = null) {
  if (!useCloudStorage) {
    throw new Error('Cloud storage not configured. Cannot migrate.');
  }

  const uploadsDir = path.join(__dirname, '../../uploads');
  if (!fs.existsSync(uploadsDir)) {
    return { success: true, migrated: 0, message: 'No uploads directory found.' };
  }
  ...
  for (const filePath of files) {
    ...
    try {
      if (!(await fileExists(relativePath))) {
        ...
        migratedCount++;
        if (callback) callback(relativePath, true);
      } else {
        if (callback) callback(relativePath, false, 'Already exists');
      }
    } catch (err) {
      logger.error('Migration failed for file', { file: relativePath, error: err.message });
      if (callback) callback(relativePath, false, err.message);
    }
  }

  return { success: true, migrated: migratedCount, total: files.length };
}
