...
async function startApprovalProcess(contentId, workflowId, userId) {
  try {
    ...
    // 🛸 Phase 15 & 16: Linguistic Auto-Assignment
    if (content.metadata?.translations && Object.keys(content.metadata.translations).length > 0) {
      logger.info('Phase 16: Detecting translations, searching for available Linguistic Reviewers');
      const team = await Team.findById(content.teamId || workflow.teamId || content.workspaceId);
      if (team) {
        const reviewers = team.members.filter(m => m.role === 'linguistic_reviewer');
        if (reviewers.length > 0) {
          // Assign the first available linguistic reviewer to a new lead stage
          const firstReviewer = reviewers[0];
          logger.info('Auto-assigning first available linguistic reviewer', { userId: firstReviewer.userId });
          
          stages.unshift({
            stageOrder: -1, // Pre-stage for linguistic review
            stageName: 'Linguistic Verification',
            status: 'pending',
            startedAt: new Date(),
            approvals: [{
              approverId: firstReviewer.userId,
              status: 'pending'
            }],
            autoApproved: false
          });
          ...
          // Original first stage becomes second
          if (stages[1]) {
            stages[1].status = 'pending';
            stages[1].startedAt = null;
          }
        }
      }
    }
    ...
    const approval = new ContentApproval({
      ...
      workflowId,
      createdBy: userId,
      currentStage: stages[0].stageOrder,
      status: 'pending',
      stages,
      assignedTo,
      history: [{
        action: 'created',
        userId,
        comment: 'Approval process started with Neural Linguistic Auto-Assignment'
      }]
    });
    ...
    await notifyStageApprovers(approval, stages[0].stageOrder);
