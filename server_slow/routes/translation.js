...
/**
 * POST /api/translation/sync/:contentId
 * Synchronize translations for content
 */
router.post('/sync/:contentId', auth, asyncHandler(async (req, res) => {
  const { contentId } = req.params;
  ...
  const result = await SyncEngine.syncContent(contentId);
  sendSuccess(res, 'Synchronization triggered', 200, result);
}));

/**
 * POST /api/translation/sync-delta/:translationId
 * Trigger delta translation for a specific translation
 */
router.post('/sync-delta/:translationId', auth, asyncHandler(async (req, res) => {
  const { translationId } = req.params;

  const result = await SyncEngine.triggerDeltaTranslation(translationId);
  sendSuccess(res, 'Delta translation complete', 200, result);
}));
