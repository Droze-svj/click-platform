...
// Get content generation status (for polling)
router.get('/:contentId/status', auth, async (req, res) => {
  try {
    const content = await Content.findOne({
      ...
      .select('status generatedContent errorMessage')
      .lean()
      .maxTimeMS(5000);
    ...
    logger.error('Get content status error', { error: error.message, contentId: req.params.contentId });
    res.status(500).json({ success: false, error: error.message });
  }
});
