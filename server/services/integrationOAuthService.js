...
// OAuth flow helpers for integrations
...
async function generateOAuthUrl(integrationId, userId, redirectUri) {
  try {
    ...
    // Store state in integration config temporarily
    ...
    logger.info('OAuth URL generated', { integrationId, userId });
    return { authUrl, state };
  } catch (error) {
    logger.error('Error generating OAuth URL', { error: error.message, integrationId });
    throw error;
  }
}
...
async function handleOAuthCallback(integrationId, code, state) {
  try {
    ...
    // Verify state
    if (integration.config.oauthState !== state) {
      throw new Error('Invalid OAuth state');
    }

    const marketplace = await IntegrationMarketplace.findById(integration.metadata?.marketplaceId);
    if (!marketplace) {
      throw new Error('Marketplace integration not found');
    }
    ...
    logger.info('OAuth callback handled', { integrationId });
    return { success: true, integration };
  } catch (error) {
    logger.error('Error handling OAuth callback', { error: error.message, integrationId });
    throw error;
  }
}
...
async function refreshOAuthToken(integrationId) {
  try {
    ...
    logger.info('OAuth token refreshed', { integrationId });
    return { success: true, expiresAt: integration.config.credentials.expiresAt };
  } catch (error) {
    logger.error('Error refreshing OAuth token', { error: error.message, integrationId });
    throw error;
  }
}
...
async function testIntegration(integrationId, testData = {}) {
  try {
    ...
    if (integration.config.credentials?.expiresAt && 
        integration.config.credentials.expiresAt < new Date()) {
    ...
    logger.info('Integration tested', { integrationId, success, responseTime });
    return {
      success,
    ...
    logger.error('Error testing integration', { error: error.message, integrationId });
