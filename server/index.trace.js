...
try {
  ...
  if (isValidDsn) {
    ...
    const sentryAiRecordInputs = process.env.SENTRY_AI_RECORD_INPUTS !== 'false';
    const sentryAiRecordOutputs = process.env.SENTRY_AI_RECORD_OUTPUTS !== 'false';
    const integrations = [];

    // AI Agent Monitoring integrations (Sentry JS SDK >= 10.28)
    // Guard each integration so older SDK versions don't break startup.
    if (typeof Sentry.openAIIntegration === 'function') {
      integrations.push(Sentry.openAIIntegration({
        recordInputs: sentryAiRecordInputs,
        recordOutputs: sentryAiRecordOutputs,
      }));
    }
    if (typeof Sentry.googleGenAIIntegration === 'function') {
