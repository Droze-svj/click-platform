              ...
              >
                ...
                <div className="px-8 pb-8">
                   <button onClick={() => handleEscalateProtocol(pkg.slug)} disabled={isCurrent || isUpgrading}
                     className={`w-full py-12 rounded-[3.5rem] text-[18px] font-black uppercase tracking-[0.8em] transition-all duration-300 shadow-3xl flex items-center justify-center gap-10 italic group/btn relative overflow-hidden active:scale-95 border-none outline-none ${
                       isCurrent ? 'bg-white/5 text-slate-500 border-2 border-white/10 cursor-not-allowed' : 'bg-white text-black hover:bg-indigo-600 hover:text-white'
                     }`}
                   >
    ...
    <div className="bg-black/60 border-2 border-white/5 p-16 rounded-[4.5rem] group/sat hover:border-indigo-500/40 transition-all duration-300 shadow-inner backdrop-blur-3xl relative overflow-hidden">
       ...
       <div className="flex items-end gap-10 mb-12 relative z-10">
          <span className="text-7xl font-black text-white italic tabular-nums leading-none tracking-tighter drop-shadow-2xl">
             {value}
             {limit !== -1 && <span className="text-slate-500 ml-4 opacity-40 text-4xl"> / {limit}</span>}
          </span>
       </div>
