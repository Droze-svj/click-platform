    ...
    return (
       <div className="flex flex-col items-center justify-center py-48 bg-[#020205] min-h-screen gap-12 backdrop-blur-3xl">
          ...
          <div className="space-y-4 text-center">
            <p className="text-[14px] font-black text-[var(--tint-indigo-fg)] uppercase tracking-[0.8em] animate-pulse italic leading-none">Deciphering Heuristic Blueprints...</p>
      ...
      <div className="flex flex-col lg:flex-row items-center justify-between gap-16 relative z-50">
        <div className="flex items-center gap-12">
          ...
          <div className="w-24 h-24 bg-[var(--tint-indigo-bg)] border-2 border-[var(--tint-indigo-edge)] rounded-[3rem] flex items-center justify-center shadow-3xl relative group overflow-hidden">
             ...
             <Layers size={48} className="text-[var(--tint-indigo-fg)] relative z-10 group-hover:scale-125 transition-transform duration-300" />
          </div>
          <div>
            <div className="flex items-center gap-6 mb-4">
              <Compass className="text-[var(--tint-indigo-fg)] animate-pulse" size={16} />
              <span className="text-[12px] font-black uppercase tracking-[0.6em] text-[var(--tint-indigo-fg)] italic leading-none">Heuristic Scaffolding HUD v14.2</span>
            </div>
            ...
            <div className={`${glassStyle} p-12 rounded-[5rem] flex flex-col gap-16 border-white/5 shadow-[0_100px_300px_rgba(0,0,0,0.8)]`}>
                    ...
                    {sectors.map((sector) => (
                      ...
                      >
                        ...
                        {selectedSector === sector ? (
                          ...
                          <ChevronDown size={20} className="text-slate-500 group-hover/s:rotate-90 transition-transform duration-300" />
                        )}
                      </button>
                    ))}
                  </div>
                  ...
                  <div className="space-y-4 max-h-[600px] overflow-y-auto custom-scrollbar pr-6">
                    {domains.map((domain) => (
                      <button
                        ...
                        className={`w-full text-left px-10 py-6 rounded-[3rem] text-[14px] font-black uppercase tracking-[0.4em] transition-all duration-300 italic flex items-center justify-between group/d ${selectedDomain === domain ? 'bg-[var(--tint-indigo-bg)] text-[var(--tint-indigo-fg)] border-2 border-indigo-500/50 shadow-[0_0_80px_rgba(99,102,241,0.2)]' : 'text-slate-500 hover:text-white hover:bg-white/[0.05]'}`}
                      >
                        {domain.toUpperCase()}
                ...
                <div className="pt-8 border-t-2 border-white/5">
                   <div className="p-10 bg-indigo-500/5 rounded-[4rem] border-2 border-indigo-500/10 shadow-inner group/status overflow-hidden relative">
                      <div className="absolute inset-0 bg-indigo-500/5 scale-x-0 group-hover/status:scale-x-100 origin-left transition-transform duration-300" />
                      <div className="relative z-10">
                        <div className="flex items-center gap-6 mb-6">
                           <Shield size={28} className="text-[var(--tint-indigo-fg)] group-hover/status:scale-110 transition-transform duration-300" />
            ...
            <div className={`${glassStyle} p-16 rounded-[6rem] h-full border-white/5 flex flex-col shadow-[0_100px_300px_rgba(0,0,0,1)]`}>
              <div className="flex flex-col md:flex-row items-center justify-between mb-20 gap-10">
                 <div className="flex items-center gap-10">
                    <div className="w-20 h-20 bg-[var(--tint-indigo-bg)] border-2 border-[var(--tint-indigo-edge)] rounded-[2.5rem] flex items-center justify-center shadow-3xl group-hover:rotate-12 transition-transform duration-300"><Terminal size={40} className="text-[var(--tint-indigo-fg)]" /></div>
                    <div>
                 ...
                 <div className="flex items-center gap-8">
                    <div className="px-10 py-4 rounded-full bg-white/[0.02] border-2 border-white/10 text-[12px] font-black text-[var(--tint-indigo-fg)] uppercase tracking-[0.4em] italic flex items-center gap-5 shadow-inner">
              ...
              {templates.length === 0 ? (
                <div className="flex-1 flex flex-col items-center justify-center text-center p-32 opacity-10 group/void">
                   <div className="w-48 h-48 bg-white/5 rounded-[6rem] border-2 border-white/5 flex items-center justify-center shadow-inner group-hover/void:scale-110 transition-transform duration-300">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 2xl:grid-cols-3 gap-16">
                  {templates.map((template, idx) => (
                    <motion.div
                      ...
                      layout
                      key={template._id}
                      className="group bg-white/[0.02] border-2 border-white/5 rounded-[5rem] p-12 hover:border-[var(--tint-indigo-edge)] hover:bg-white/[0.05] transition-all duration-300 shadow-[0_50px_100px_rgba(0,0,0,0.8)] flex flex-col min-h-[580px] relative overflow-hidden h-full"
                    >
                      <div className="absolute top-0 right-0 p-12 opacity-0 group-hover:opacity-10 transition-opacity duration-300 pointer-events-none -z-10"><Hexagon size={240} className="text-white" /></div>
                      
                      {template.preview?.thumbnail && (
                        <div className="relative mb-10 rounded-[3rem] overflow-hidden aspect-video border-2 border-white/10 shadow-3xl group-hover:border-[var(--tint-indigo-edge)] transition-all duration-300 group-hover:shadow-[0_0_100px_rgba(99,102,241,0.2)]">
                          <img
                            src={template.preview.thumbnail}
                            alt={template.name}
                            className="w-full h-full object-cover grayscale-[0.8] group-hover:grayscale-0 transition-all duration-500 group-hover:scale-110"
                          />
                      ...
                      <div className="flex justify-between items-start mb-8 relative z-10 px-4">
                        <div className="space-y-2">
                          <p className="text-[10px] font-black text-[var(--tint-indigo-fg)] uppercase tracking-[0.6em] italic">Blueprint_ID: {template._id.slice(-8).toUpperCase()}</p>
                          <h3 className="text-4xl font-black text-white italic uppercase tracking-tighter leading-none group-hover:text-[var(--tint-indigo-fg)] transition-colors duration-300 line-clamp-1">{template.name.toUpperCase()}</h3>
                        </div>
                        {template.isPublic && (
                      ...
                      <div className="px-4 flex-1">
                         <p className="text-[14px] text-slate-500 font-black uppercase tracking-widest mb-12 italic line-clamp-3 leading-tight opacity-40 group-hover:opacity-100 transition-opacity duration-300">{template.description}</p>
                      </div>
                      
                      <div className="mt-auto space-y-10 relative z-10">
                        <div className="flex items-center gap-6 text-[11px] font-black text-slate-500 uppercase tracking-widest italic border-t-2 border-white/5 pt-12 group-hover:text-[var(--tint-indigo-fg)] transition-colors duration-300 px-4">
                          <div className="flex items-center gap-3 bg-white/5 px-4 py-2 rounded-2xl border-2 border-white/5 group-hover:border-[var(--tint-indigo-edge)]">{template.category.toUpperCase()}</div>
                          <div className="w-2 h-2 rounded-full bg-slate-500 opacity-20" />
                          <div className="flex items-center gap-3 bg-white/5 px-4 py-2 rounded-2xl border-2 border-white/5 group-hover:border-[var(--tint-indigo-edge)]">{template.niche.toUpperCase()}</div>
                          <div className="w-2 h-2 rounded-full bg-slate-500 opacity-20" />
                        ...
                        {template.tags && template.tags.length > 0 && (
                          <div className="flex flex-wrap gap-5 px-4">
                            {template.tags.slice(0, 3).map((tag, index) => (
                              <span key={index} className="text-[11px] font-black text-slate-500 uppercase italic group-hover:text-white transition-colors duration-300 hover:text-[var(--tint-indigo-fg)] cursor-default">
                                #{tag.toUpperCase()}
                              </span>
                            ))}
                          </div>
