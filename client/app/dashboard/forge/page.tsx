        ...
        <div className="max-w-[1600px] mx-auto relative z-10 space-y-12">
          ...
          <header className="flex flex-col lg:flex-row items-center justify-between gap-8 border-b border-white/5 pb-12">
            <div className="flex items-center gap-8">
              ...
              <div className="w-16 h-16 bg-[var(--tint-indigo-bg)] border-2 border-[var(--tint-indigo-edge)] rounded-2xl flex items-center justify-center shadow-2xl relative group">
                ...
                <Zap size={32} className="text-[var(--tint-indigo-fg)] relative z-10 animate-pulse" />
              </div>
              <div>
                <div className="flex items-center gap-3 mb-1">
                  <Cpu size={14} className="text-[var(--tint-indigo-fg)]/80" />
                  <span className="text-[11px] font-black uppercase tracking-[0.4em] text-[var(--tint-indigo-fg)] italic">Neural Forge v4.2</span>
                </div>
            ...
            <div className="hidden xl:flex items-center gap-12 text-right">
              ...
              <div className="bg-white/[0.02] border border-white/5 px-6 py-3 rounded-2xl shadow-xl">
                <p className="text-[10px] font-black text-[var(--tint-indigo-fg)] uppercase tracking-widest italic leading-none mb-2">Engine State</p>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-12 gap-12">
            ...
            <div className="xl:col-span-9 space-y-12">
              ...
              <section className="space-y-6">
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 rounded-xl bg-[var(--tint-indigo-bg)] border border-[var(--tint-indigo-edge)] flex items-center justify-center text-[var(--tint-indigo-fg)]">
                    <Sparkles size={20} />
                  </div>
                  <div>
            ...
            <div className="xl:col-span-3 space-y-8">
              ...
              <div className={`${glassStyle} rounded-[2.5rem] p-8 bg-black/40 flex flex-col min-h-[500px]`}>
                <div className="flex items-center gap-4 mb-8">
                  <div className="w-12 h-12 rounded-2xl bg-[var(--tint-indigo-bg)] border border-[var(--tint-indigo-edge)] flex items-center justify-center">
                    <Network size={24} className="text-[var(--tint-indigo-fg)]" />
                  </div>
                  <div>
                ...
                <div className="flex-1 space-y-4 overflow-y-auto max-h-[600px] pr-2 custom-scrollbar">
                  {loadingHistory ? (
                    <div className="py-20 text-center space-y-4 opacity-50">
                      <Loader2 size={32} className="text-[var(--tint-indigo-fg)] animate-spin mx-auto" />
                      <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest italic">Scanning Vault...</p>
                    </div>
                  ) : history.length > 0 ? (
                    history.map((item) => (
                      <div key={item._id} className="p-5 rounded-2xl bg-white/[0.02] border border-white/5 hover:border-[var(--tint-indigo-edge)] transition-all cursor-pointer group">
                        <div className="flex items-center justify-between mb-2">
                          <span className="text-[9px] font-black text-slate-600 uppercase tracking-widest">
                            {new Date(item.createdAt).toLocaleDateString([], { month: 'short', day: 'numeric' })}
                          </span>
                          <div className="px-2 py-0.5 rounded-full bg-[var(--tint-emerald-bg)] border border-[var(--tint-emerald-edge)] text-[8px] font-black text-[var(--tint-emerald-fg)] uppercase tracking-widest">SUCCESS</div>
                        </div>
                        <p className="text-sm font-bold text-white tracking-tight group-hover:text-[var(--tint-indigo-fg)] transition-colors line-clamp-1">{item.title}</p>
                      </div>
                    ))
                  ) : (
              ...
              <div className={`${glassStyle} rounded-[2.5rem] p-8 bg-black/40`}>
                <div className="flex items-center gap-2 mb-6">
                  <Gauge className="text-[var(--tint-indigo-fg)]" size={16} />
                  <span className="text-[10px] font-black text-[var(--tint-indigo-fg)] uppercase tracking-widest italic leading-none">Resource Pulse</span>
                </div>
                <div className="flex items-end gap-1 h-12">
                  {[...Array(12)].map((_, i) => (
                    <motion.div 
                      ...
                      className="flex-1 bg-[var(--tint-indigo-bg)] rounded-t-sm"
                    />
                  ))}
                </div>
              </div>
