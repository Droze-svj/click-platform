...
export default function KineticWorkspaceHubPage() {
  ...
  if (loading) return (
     <div className="flex flex-col items-center justify-center py-48 bg-[#020205] min-h-screen font-inter">
        ...
        <span className="text-[12px] font-black text-slate-800 uppercase tracking-[0.6em] animate-pulse italic">Synchronizing Cluster Matrix...</span>
     </div>
  )
      ...
      <div className="min-h-screen relative z-10 pb-48 px-10 pt-16 max-w-[1700px] mx-auto space-y-24 font-inter">
        ...
        <header className="flex flex-col lg:flex-row items-center justify-between gap-12 relative z-50">
              ...
              <div>
                 <div className="flex items-center gap-6 mb-4">
                   ...
                   <div className="flex items-center gap-3 px-6 py-2 rounded-full bg-black/60 border-2 border-white/5 shadow-inner">
                       ...
                       <span className="text-[10px] font-black text-slate-800 tracking-widest uppercase italic leading-none">{projects.length} ACTIVE_CLUSTERS</span>
                   </div>
                 </div>
                 ...
                 <p className="text-slate-800 text-[13px] uppercase font-black tracking-[0.6em] mt-5 italic leading-none">Stratified mission nexus for coordinate cluster management and temporal trajectory mapping.</p>
              </div>
           </div>
           ...
           >
             ...
             <Plus size={32} className="group-hover:rotate-90 transition-transform duration-1000" /> INITIATE_CLUSTER
           </button>
        </header>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 relative z-10">
           ...
           <div className="lg:col-span-3 space-y-12">
              <div className="flex items-center justify-between px-8 border-l-4 border-indigo-500/30 ml-2">
                 ...
                 <Activity size={20} className="text-slate-900 animate-pulse" />
              </div>
              <div className="space-y-8 max-h-[1200px] overflow-y-auto pr-6 custom-scrollbar">
                 {projects.map((p) => (
                   <motion.button
                     ...
                     className={`w-full text-left rounded-[4.5rem] p-12 border-2 transition-all duration-1000 group relative overflow-hidden shadow-3xl ${
                   ...
                   >
                     <div className="absolute top-0 right-0 p-12 opacity-0 group-hover:opacity-[0.05] transition-opacity duration-1000 pointer-events-none group-hover:rotate-12 group-hover:scale-150"><Target size={150} className="text-white" /></div>
                     <div className="flex justify-between items-start mb-8 relative z-10">
                        <span className={`text-[24px] font-black tracking-tighter uppercase italic truncate pr-4 transition-colors duration-1000 ${selectedProject?._id === p._id ? 'text-white underline decoration-indigo-500 decoration-4 underline-offset-8' : 'text-slate-700 group-hover:text-white'}`}>{p.name}</span>
                     ...
                     <div className="flex items-center gap-10 relative z-10">
                        <div className="flex items-center gap-3 text-[11px] font-black text-slate-800 uppercase italic bg-black/40 px-6 py-2 rounded-full border border-white/5 shadow-inner">
                           <Activity size={16} className={selectedProject?._id === p._id ? 'text-indigo-400 animate-pulse' : ''} /> {p.progress}% SAT
                        </div>
                        <div className="flex items-center gap-3 text-[11px] font-black text-slate-800 uppercase italic">
                 ...
                 {!selectedProject ? (
                    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className={`${glassStyle} rounded-[6.5rem] p-32 text-center flex flex-col items-center justify-center min-h-[850px] border-2 border-white/5 shadow-[0_80px_200px_rgba(0,0,0,0.8)] relative group overflow-hidden`}>
                       ...
                       <div className="w-48 h-48 bg-black/60 border-2 border-white/10 rounded-[5.5rem] flex items-center justify-center mb-16 shadow-3xl relative overflow-hidden group-hover:border-indigo-500/50 transition-all duration-700">
                          <Terminal size={80} className="text-slate-900 group-hover:text-indigo-400 transition-all duration-1000 group-hover:scale-125 group-hover:rotate-12" />
                       </div>
                    ...
                    <motion.div initial={{ opacity: 0, y: 100 }} animate={{ opacity: 1, y: 0 }} className="space-y-20">
                       ...
                       <div className={`${glassStyle} rounded-[7rem] p-24 border-2 border-white/10 relative overflow-hidden shadow-[0_100px_250px_rgba(0,0,0,1)] group/hud`}>
                          <div className="absolute top-0 right-0 p-32 opacity-[0.05] pointer-events-none group-hover/hud:opacity-[0.1] transition-opacity duration-1000"><Activity size={600} className="text-indigo-400" /></div>
                          ...
                          <div className="flex flex-col xl:flex-row justify-between items-start gap-20 relative z-10">
                             <div className="max-w-4xl space-y-12">
                                <div className="flex items-center gap-10">
                                   ...
                                   <div className="flex items-center gap-5 text-[12px] font-black text-slate-800 uppercase tracking-[0.4em] italic bg-black/60 px-10 py-4 rounded-3xl border-2 border-white/5 shadow-inner backdrop-blur-3xl">
                                      <Database size={18} className="text-indigo-500" /> NODE_ID: {dashboard._id.slice(-16).toUpperCase()}
                                   </div>
                                </div>
                                <h2 className="text-8xl font-black text-white italic uppercase tracking-tighter leading-none pr-10 drop-shadow-[0_20px_50px_rgba(0,0,0,0.5)] group-hover/hud:text-indigo-400 transition-colors duration-1000">{dashboard.name}</h2>
                             ...
                             <div className="flex flex-col gap-12 w-full xl:w-auto">
                                <div className="space-y-6">
                                   <label className="text-[14px] font-black text-slate-900 uppercase tracking-[0.8em] italic leading-none ml-10">CLUSTER_CALIBRATION</label>
                                   <div className="relative group/select">
                                      ...
                                      >
                                        <option value="planning" className="bg-slate-950">PHASE: STRAT_INIT</option>
                                        <option value="active" className="bg-slate-950">PHASE: KINETIC_FLOW</option>
                                        <option value="on_hold" className="bg-slate-950">PHASE: STATIC_STASIS</option>
                                        <option value="completed" className="bg-slate-950">PHASE: COGNITIVE_DONE</option>
                                      </select>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-16 mt-32 relative z-10">
                             <div className="bg-black/60 border-2 border-white/5 rounded-[5rem] p-16 space-y-12 group/stat hover:border-indigo-500/50 transition-all duration-1000 shadow-inner backdrop-blur-3xl">
                                <div className="flex justify-between items-center px-4">
                                   <p className="text-[15px] text-slate-800 font-black uppercase tracking-[0.6em] italic leading-none">Cluster_Saturation</p>
                                   <Activity size={28} className="text-slate-950 group-hover/stat:text-indigo-400 transition-colors duration-1000 animate-pulse" />
                                </div>
                                <div className="flex items-end gap-8 px-4">
                                   <span className="text-8xl font-black text-white italic tabular-nums leading-none tracking-tighter drop-shadow-2xl">{dashboard.progress}%</span>
                                   <span className="text-[13px] font-black text-slate-900 uppercase mb-4 tracking-[0.4em] italic">NEURAL_SAT</span>
                                </div>
                             ...
                             <div className="bg-black/60 border-2 border-white/5 rounded-[5rem] p-16 space-y-12 group/stat hover:border-emerald-500/50 transition-all duration-1000 shadow-inner backdrop-blur-3xl">
                                <div className="flex justify-between items-center px-4">
                                   <p className="text-[15px] text-slate-800 font-black uppercase tracking-[0.6em] italic leading-none">Quantum_Forecast</p>
                                   <Radio size={28} className="text-slate-950 group-hover/stat:text-emerald-400 transition-colors animate-pulse duration-1000" />
                                </div>
                                {dashboard.aiPredictedCompletionDate ? (
                                  ...
                                  <div className="h-40 flex items-center justify-center border-4 border-dashed border-white/5 rounded-[4rem]">
                                     <p className="text-[18px] font-black text-slate-950 uppercase tracking-[0.8em] italic">SIGNAL_OFFLINE</p>
                                  </div>
                                )}
                             </div>
                             ...
                             <motion.button
                               ...
                               onClick={runPredict}
                               disabled={predicting}
                               className="rounded-[5rem] bg-white border-8 border-transparent hover:border-indigo-500/30 text-black p-16 flex flex-col items-center justify-center gap-10 shadow-[0_80px_200px_rgba(255,255,255,0.1)] hover:shadow-indigo-500/50 transition-all duration-1000 disabled:opacity-30 group/btn relative overflow-hidden"
                             >
                                <div className="absolute inset-0 bg-gradient-to-tr from-indigo-500/5 via-transparent to-transparent opacity-0 group-hover/btn:opacity-100 transition-opacity duration-1000" />
                                <div className="w-24 h-24 rounded-[3rem] bg-black border-2 border-white/20 flex items-center justify-center shadow-3xl group-hover/btn:rotate-[360deg] transition-transform duration-[2s] relative z-10">
                       ...
                       <section className={`${glassStyle} rounded-[7rem] p-24 border-2 border-white/5 shadow-[0_80px_200px_rgba(0,0,0,0.8)] relative overflow-hidden group/terminal`}>
                          <div className="absolute top-0 right-0 p-32 opacity-[0.03] pointer-events-none group-hover/terminal:opacity-[0.08] transition-opacity duration-1000"><Terminal size={500} className="text-white" /></div>
                          <div className="flex flex-col xl:flex-row justify-between items-center mb-24 gap-12 relative z-10">
                             <div className="flex items-center gap-12">
                                <div className="w-24 h-24 bg-indigo-500/10 border-2 border-indigo-500/20 rounded-[3rem] flex items-center justify-center shadow-3xl group-hover/terminal:scale-110 transition-transform duration-1000"><Target size={48} className="text-indigo-400" /></div>
                                <div>
                                   <h3 className="text-6xl font-black text-white italic uppercase tracking-tighter leading-none mb-4">Objective Terminal</h3>
                                   <p className="text-[14px] text-slate-800 font-black uppercase tracking-[0.6em] italic leading-none">Stratified mission nodes and temporal trajectory lattices.</p>
                                </div>
                             </div>
                             ...
                             <div className="flex items-center gap-8 p-5 bg-black/60 rounded-[4.5rem] border-2 border-white/10 shadow-inner backdrop-blur-3xl">
                                <button onClick={() => setViewMode('list')} className={`px-14 py-8 rounded-[3.5rem] text-[13px] font-black uppercase tracking-[0.6em] transition-all duration-1000 italic border-none outline-none ${viewMode === 'list' ? 'bg-white text-black shadow-3xl scale-110' : 'text-slate-800 hover:text-white hover:bg-white/5'}`}>LATTICE_VIEW</button>
                                <button onClick={() => setViewMode('gantt')} className={`px-14 py-8 rounded-[3.5rem] text-[13px] font-black uppercase tracking-[0.6em] transition-all duration-1000 italic border-none outline-none flex items-center gap-8 ${viewMode === 'gantt' ? 'bg-white text-black shadow-3xl scale-110' : 'text-slate-800 hover:text-white hover:bg-white/5'}`}><BarChart3 size={28} /> TEMPORAL_MESH</button>
                                <div className="w-[2px] h-10 bg-white/5 mx-2" />
                                <button onClick={() => setShowAddMilestone(true)} className="px-14 py-8 text-indigo-400 text-[13px] font-black uppercase tracking-[0.6em] italic hover:text-white transition-all group/add border-none outline-none bg-transparent flex items-center gap-6"><Plus size={28} className="group-hover:rotate-90 transition-transform duration-1000" /> NODE_INJECT</button>
                             </div>
                          </div>
                                     ...
                                     return (
                                       <div key={m._id} className="flex items-center gap-24 group min-w-[1400px]">
                                         <div className="w-96 truncate flex items-center gap-10">
                                           <div className={`w-5 h-5 rounded-full shadow-3xl ${m.completedAt ? 'bg-emerald-500 shadow-[0_0_20px_rgba(16,185,129,1)]' : m.criticalPathInfo?.isOnCriticalPath ? 'bg-indigo-500 animate-pulse shadow-[0_0_25px_rgba(99,102,241,1)]' : 'bg-slate-950 border-2 border-white/5 shadow-inner'}`} />
                                           <span className={`text-[26px] font-black italic uppercase tracking-tighter transition-colors duration-1000 group-hover:text-indigo-400 ${m.completedAt ? 'text-slate-800' : 'text-white'}`}>{m.title}</span>
                                         </div>
                                         ...
                                         <div className="w-40 text-right">
                                            <span className="text-[16px] font-black text-slate-900 uppercase tracking-[0.4em] tabular-nums italic">{m.estimatedDays || 0} CYC</span>
                                         </div>
                                       </div>
                                     )
                                   })}
                                ...
                                <div className="space-y-10">
                                   {dashboard.milestones.map((m) => (
                                     <motion.div
                                       layout
                                       key={m._id}
                                       className={`flex items-center gap-16 p-14 rounded-[6rem] border-2 transition-all duration-1000 relative overflow-hidden group shadow-3xl backdrop-blur-3xl ${
                                     ...
                                     >
                                        <div className="absolute top-0 right-0 p-24 opacity-0 group-hover:opacity-[0.05] transition-opacity duration-[2s] pointer-events-none -rot-12 scale-150"><Target size={300} className="text-white" /></div>
                                        ...
                                        <div className="flex-1 min-w-0">
                                           <div className="flex items-center gap-10 mb-6">
                                              <h4 className={`text-[36px] font-black italic uppercase tracking-tighter truncate leading-none transition-colors duration-1000 ${m.completedAt ? 'text-slate-800' : 'text-white group-hover:text-indigo-400'}`}>{m.title}</h4>
                                        ...
                                        <div className="flex items-center gap-16 shrink-0 pr-8">
                                           {m.dueDate && (
                                             <div className="flex flex-col items-end">
                                                <span className="text-[13px] font-black text-slate-900 uppercase tracking-[0.6em] italic mb-3">THRESHOLD</span>
                                                <span className="text-[20px] font-black text-slate-700 uppercase italic leading-none tabular-nums opacity-60 group-hover:opacity-100 transition-all duration-1000">{new Date(m.dueDate).toLocaleDateString().toUpperCase()}</span>
                                             </div>
                                           )}
                                           <div className="h-24 w-[2px] bg-white/5" />
                                           <button onClick={() => setEditingMilestone(m)} className="w-20 h-20 bg-white/[0.03] border-2 border-white/10 rounded-3xl flex items-center justify-center text-slate-800 hover:text-white hover:bg-white/10 transition-all duration-700 shadow-3xl active:scale-90 hover:border-indigo-500/50 backdrop-blur-3xl group/edit"><Pencil size={32} className="group-hover/edit:rotate-12 transition-transform" /></button>
                                        </div>
                                     </motion.div>
                                   ))}
                                </div>
        ...
        <AnimatePresence>
           {showCreate && (
              <div className="fixed inset-0 z-50 flex items-center justify-center p-10 backdrop-blur-3xl">
                 <motion.div initial={{ opacity: 0, scale: 0.9, y: 100 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 100 }} className={`${glassStyle} w-full max-w-4xl rounded-[7rem] p-24 border-2 border-white/10 shadow-[0_100px_300px_rgba(0,0,0,1)] relative overflow-hidden`}>
                    <div className="absolute top-0 right-0 p-32 opacity-[0.05] pointer-events-none group-hover:opacity-[0.1] transition-opacity duration-1000 shadow-inner"><Zap size={400} className="text-white" /></div>
                    <div className="flex justify-between items-center mb-24 relative z-10">
                       <div>
                          ...
                          <p className="text-[14px] text-slate-800 font-black uppercase tracking-[0.6em] italic leading-none">NODE_MATRIX_UPLINK_INITIALIZATION</p>
                       </div>
                       <button onClick={() => setShowCreate(false)} title="Close Modal" className="w-20 h-20 bg-white/5 border-2 border-white/5 rounded-3xl flex items-center justify-center text-slate-800 hover:text-white transition-all shadow-3xl active:scale-90 hover:border-white/20 hover:rotate-90 duration-700 backdrop-blur-3xl"><X size={40} /></button>
                    </div>
                    
                    <div className="space-y-16 relative z-10">
                       <div className="space-y-8">
                          <label htmlFor="cluster-id" className="text-[14px] font-black text-slate-900 uppercase tracking-[0.8em] italic ml-10">CLUSTER_IDENTIFIER</label>
                          <input
                            id="cluster-id"
                            type="text"
                            ...
                            className="w-full bg-black/80 border-2 border-white/5 rounded-[4rem] px-16 py-10 text-[24px] font-black text-white uppercase tracking-tighter italic placeholder:text-slate-950 focus:outline-none focus:border-indigo-500/50 transition-all shadow-inner backdrop-blur-3xl"
                          />
                       </div>
                       <div className="space-y-8">
                          <label htmlFor="mission-params" className="text-[14px] font-black text-slate-900 uppercase tracking-[0.8em] italic ml-10">MISSION_PARAMETERS</label>
                          <textarea
                            id="mission-params"
                            ...
                            className="w-full bg-black/80 border-2 border-white/5 rounded-[5rem] px-16 py-10 text-[18px] font-black text-indigo-400 uppercase tracking-widest italic placeholder:text-slate-950 focus:outline-none focus:border-indigo-500/50 transition-all shadow-inner resize-none backdrop-blur-3xl"
                          />
                       </div>

                       <div className="flex gap-10 pt-10">
                          <button onClick={() => setShowCreate(false)} className="flex-1 py-10 rounded-[4rem] bg-white/5 border-2 border-white/5 text-slate-800 text-[18px] font-black uppercase tracking-[0.8em] italic hover:text-white hover:bg-white/10 transition-all shadow-3xl backdrop-blur-3xl">ABORT_SYNCH</button>
           ...
           {showAddMilestone && (
              <div className="fixed inset-0 z-50 flex items-center justify-center p-10 backdrop-blur-3xl">
                 <motion.div initial={{ opacity: 0, scale: 0.9, y: 100 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 100 }} className={`${glassStyle} w-full max-w-4xl rounded-[7rem] p-24 border-2 border-white/10 shadow-[0_100px_300px_rgba(0,0,0,1)] relative overflow-hidden`}>
                    <div className="absolute top-0 right-0 p-32 opacity-[0.05] pointer-events-none group-hover:opacity-[0.1] transition-opacity duration-1000 shadow-inner"><Target size={400} className="text-white" /></div>
                    <div className="flex justify-between items-center mb-20 relative z-10">
                       <div>
                          ...
                          <p className="text-[14px] text-slate-800 font-black uppercase tracking-[0.6em] italic leading-none">APPENDING_MISSION_CRITICAL_ANCHOR</p>
                       </div>
                       <button onClick={() => setShowAddMilestone(false)} title="Close Modal" className="w-20 h-20 bg-white/5 border-2 border-white/5 rounded-3xl flex items-center justify-center text-slate-800 hover:text-white transition-all shadow-3xl active:scale-90 hover:border-white/20 hover:rotate-90 duration-700 backdrop-blur-3xl"><X size={40} /></button>
                    </div>

                    <div className="space-y-12 relative z-10">
                       <div className="space-y-6">
                          <label htmlFor="node-id-new" className="text-[14px] font-black text-slate-900 uppercase tracking-[0.8em] italic ml-10">NODE_IDENTIFIER</label>
                          <input
                            id="node-id-new"
                            type="text"
                            ...
                            className="w-full bg-black/80 border-2 border-white/5 rounded-[4rem] px-16 py-8 text-[22px] font-black text-white uppercase tracking-tighter italic placeholder:text-slate-950 focus:outline-none focus:border-indigo-500/50 transition-all shadow-inner backdrop-blur-3xl"
                          />
                       </div>

                       <div className="space-y-6">
                          <label htmlFor="cycle-new" className="text-[14px] font-black text-slate-900 uppercase tracking-[0.8em] italic ml-10">CYCLE_ESTIMATION [DAYS]</label>
                          <input
                            id="cycle-new"
                            type="number"
                            min={1}
                       ...
                       <div className="flex gap-10 pt-8">
                          <button onClick={() => setShowAddMilestone(false)} className="flex-1 py-8 rounded-[4rem] bg-white/5 border-2 border-white/5 text-slate-800 text-[18px] font-black uppercase tracking-[0.8em] italic hover:text-white hover:bg-white/10 transition-all shadow-3xl backdrop-blur-3xl">CANCEL_OPS</button>
           ...
           {editingMilestone && (
              <div className="fixed inset-0 z-50 flex items-center justify-center p-10 backdrop-blur-3xl">
                 <motion.div initial={{ opacity: 0, scale: 0.9, y: 100 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 100 }} className={`${glassStyle} w-full max-w-4xl rounded-[7rem] p-24 border-2 border-white/10 shadow-[0_100px_300px_rgba(0,0,0,1)] relative overflow-hidden`} onClick={e => e.stopPropagation()}>
                    <div className="absolute top-0 right-0 p-32 opacity-[0.05] pointer-events-none group-hover:opacity-[0.1] transition-opacity duration-[2s] shadow-inner"><Pencil size={400} className="text-white" /></div>
                    <div className="flex justify-between items-center mb-20 relative z-10">
                       <div>
                          <h2 className="text-6xl font-black text-white italic uppercase tracking-tighter leading-none mb-4">Calibrate Node</h2>
                          <p className="text-[14px] text-slate-800 font-black uppercase tracking-[0.6em] italic leading-none">MODIFYING_OBJECTIVE_VECTOR</p>
                       </div>
                       <button onClick={() => setEditingMilestone(null)} title="Close Modal" className="w-20 h-20 bg-white/5 border-2 border-white/5 rounded-3xl flex items-center justify-center text-slate-800 hover:text-white transition-all shadow-3xl active:scale-90 hover:border-white/20 hover:rotate-90 duration-700 backdrop-blur-3xl"><X size={40} /></button>
                    </div>

                    <div className="space-y-12 relative z-10">
                       <div className="space-y-6">
                          <label htmlFor="node-id-edit" className="text-[14px] font-black text-slate-900 uppercase tracking-[0.8em] italic ml-10">NODE_IDENTIFIER</label>
                          <input
                            id="node-id-edit"
                            type="text"
                            value={editTitle}
                       ...
                       <div className="grid grid-cols-2 gap-10">
                          <div className="space-y-6">
                             <label htmlFor="threshold-edit" className="text-[14px] font-black text-slate-900 uppercase tracking-[0.8em] italic ml-10">TEMPORAL_THRESHOLD</label>
                             <input
                               id="threshold-edit"
                               type="date"
                               value={editDueDate}
                          ...
                          <div className="space-y-6">
                             <label htmlFor="cycle-edit" className="text-[14px] font-black text-slate-900 uppercase tracking-[0.8em] italic ml-10">CYCLE_ESTIMATION</label>
                             <input
                               id="cycle-edit"
                               type="number"
                       ...
                       <div className="flex gap-10 pt-8">
                          <button onClick={() => setEditingMilestone(null)} className="flex-1 py-8 rounded-[4rem] bg-white/5 border-2 border-white/5 text-slate-800 text-[18px] font-black uppercase tracking-[0.8em] italic hover:text-white hover:bg-white/10 transition-all shadow-3xl backdrop-blur-3xl">ABORT_CALIBRATION</button>
