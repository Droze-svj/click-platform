...
export default function AxiomRepositoryPage() {
  ...
  if (loading) return (
     <div className="flex flex-col items-center justify-center py-48 bg-[var(--page-bg)] min-h-screen">
        ...
        <span className="text-[16px] font-black text-[var(--text-dim)] uppercase tracking-[1em] animate-pulse italic">Scanning Axiom Index...</span>
     </div>
  )
        ...
        <header className="flex flex-col lg:flex-row items-center justify-between gap-16 relative z-50">
           <div className="flex items-center gap-12">
              ...
              <div>
                 ...
                 <p className="text-[var(--text-dim)] text-sm md:text-base font-medium leading-relaxed max-w-2xl">Every clip, B-roll asset, font, and template Click has saved for you. Search, filter, drag any item straight into the editor.</p>
              </div>
           </div>

           <div className="flex items-center gap-8">
              <button onClick={() => setSelectMode(!selectMode)}
                className={`px-16 py-8 border-[3px] rounded-[3.5rem] text-[15px] font-black uppercase tracking-[0.6em] transition-all duration-300 italic shadow-2xl active:scale-95 ${selectMode ? 'bg-indigo-600 text-white border-indigo-400 shadow-[0_40px_100px_rgba(99,102,241,0.4)] scale-105' : 'bg-white/[0.02] border-white/10 text-[var(--text-dim)] hover:text-white hover:bg-white/10'}`}
              >
                {selectMode ? 'ABORT_NODE_SELECT' : 'SEQUENTIAL_NODE_SELECT'}
              </button>
              <button onClick={() => setShowCreateFolder(true)}
                className="px-16 py-8 bg-white text-black rounded-[3.5rem] text-[15px] font-black uppercase tracking-[0.6em] shadow-[0_60px_120px_rgba(255,255,255,0.1)] hover:bg-indigo-500 hover:text-white transition-all duration-300 flex items-center gap-8 italic active:scale-95 border-none group">
                <Plus size={28} className="group-hover:rotate-180 transition-transform duration-300" />
                INITIALIZE_CAPSULE
              </button>
           </div>
        </header>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-4 gap-20 relative z-10">
             ...
             <div className={`${glassStyle} p-12 rounded-[6rem] space-y-20 border-white/5 shadow-[0_60px_100px_rgba(0,0,0,0.6)] bg-black/40`}>
                ...
                <div className="relative group/search">
                   <div className="absolute inset-y-0 left-10 flex items-center pointer-events-none">
                      <Search size={28} className="text-[var(--text-dim)] group-focus-within/search:text-indigo-400 transition-colors" />
                   </div>
                   <input type="text" placeholder="SCAN_REPOSITORY_INDEX..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
                     className="w-full bg-black/60 border-2 border-white/5 rounded-[3.5rem] pl-24 pr-10 py-8 text-[16px] font-black text-white uppercase tracking-widest placeholder:text-[var(--text-dim)] focus:outline-none focus:border-indigo-500/50 transition-all italic shadow-inner" 
                   />
                </div>
                ...
                <div className="space-y-12">
                   <div className="flex items-center gap-6 mb-4 border-l-8 border-indigo-500/20 pl-6">
                      ...
                      <h3 className="text-[14px] font-black text-[var(--text-dim)] uppercase tracking-[0.6em] italic leading-none">Lattice_Structure</h3>
                   </div>
                   <div className="space-y-6">
                     <button onClick={() => setSelectedFolder(null)}
                       className={`w-full group text-left px-10 py-6 rounded-[2.5rem] text-[13px] font-black uppercase tracking-[0.4em] transition-all duration-300 flex items-center justify-between italic ${selectedFolder === null ? 'bg-white text-black shadow-2xl scale-105' : 'text-[var(--text-dim)] hover:text-white hover:bg-white/[0.04]'}`}
                     >
                       FULL_MATRIX_GRID
                       <Globe size={24} className={selectedFolder === null ? 'text-black' : 'text-[var(--text-dim)] group-hover:text-indigo-400 transition-colors'} />
                     </button>
                     {folders.map((folder) => (
                       <button key={folder._id} onClick={() => setSelectedFolder(folder._id)}
                         className={`w-full group text-left px-10 py-6 rounded-[2.5rem] text-[13px] font-black uppercase tracking-[0.4em] transition-all duration-300 flex items-center gap-8 italic ${selectedFolder === folder._id ? 'bg-white text-black shadow-2xl scale-105' : 'text-[var(--text-dim)] hover:text-white hover:bg-white/[0.04]'}`}
                       >
                         ...
                         <ChevronRight size={24} className={`opacity-0 transition-opacity duration-300 ${selectedFolder === folder._id ? 'opacity-100' : 'group-hover:opacity-40'}`} />
                       </button>
                     ))}
                   </div>
                </div>
                ...
                <div className="space-y-10">
                   <div className="flex items-center gap-6 mb-4 border-l-8 border-indigo-500/20 pl-6">
                      ...
                      <h3 className="text-[14px] font-black text-[var(--text-dim)] uppercase tracking-[0.6em] italic leading-none">Metadata_Axioms</h3>
                   </div>
                   <div className="flex flex-wrap gap-4">
                     {tags.map((tag) => (
                       <button key={tag} onClick={() => setSelectedTag(selectedTag === tag ? null : tag)}
                         className={`px-8 py-4 rounded-[2rem] text-[12px] font-black uppercase tracking-[0.2em] transition-all duration-300 border-2 italic ${selectedTag === tag ? 'bg-indigo-600 text-white border-indigo-400 shadow-[0_0_40px_rgba(99,102,241,0.5)]' : 'bg-black/40 text-[var(--text-dim)] border-white/5 hover:border-white/20 hover:text-white'}`}
                       >
                         #{tag.toUpperCase()}
                       </button>
                     ))}
                ...
                <div className="space-y-10">
                   <div className="flex items-center gap-6 mb-4 border-l-8 border-indigo-500/20 pl-6">
                      ...
                      <h3 className="text-[14px] font-black text-[var(--text-dim)] uppercase tracking-[0.6em] italic leading-none">Logic_Archetypes</h3>
                   </div>
                   <div className="space-y-6">
                     {categories.map((category) => (
                       <button key={category} onClick={() => setSelectedCategory(selectedCategory === category ? null : category)}
                         className={`w-full text-left px-10 py-6 rounded-[2.5rem] text-[13px] font-black uppercase tracking-[0.4em] transition-all duration-300 italic ${selectedCategory === category ? 'bg-indigo-600/10 text-indigo-400 border border-indigo-500/20 shadow-2xl' : 'text-[var(--text-dim)] hover:text-white hover:bg-white/[0.04]'}`}
                       >
                ...
                <div className="pt-12 border-t border-white/10">
                  <button onClick={() => setShowFavorites(!showFavorites)}
                    className={`w-full group flex items-center gap-8 px-12 py-8 rounded-[3.5rem] text-[14px] font-black uppercase tracking-[0.6em] transition-all duration-300 italic shadow-[0_40px_100px_rgba(0,0,0,0.6)] ${showFavorites ? 'bg-amber-500 text-black shadow-[0_30px_80px_rgba(245,158,11,0.5)] scale-105 border-none' : 'bg-black/40 text-[var(--text-dim)] hover:text-amber-500 border border-white/5'}`}
                  >
                    <Star size={32} fill={showFavorites ? 'currentColor' : 'none'} className={showFavorites ? 'animate-pulse' : 'group-hover:rotate-180 transition-transform duration-300'} /> AXIOMATIC_PRIMARIES
                  </button>
                </div>
             </div>
          </aside>
          ...
          <main className="lg:col-span-3 space-y-16">
             <div className={`${glassStyle} p-20 rounded-[7rem] min-h-[1100px] border-white/5 relative overflow-hidden shadow-[0_80px_200px_rgba(0,0,0,0.8)] flex flex-col bg-black/40`}>
                <div className="absolute top-0 right-0 p-32 opacity-[0.02] pointer-events-none group-hover:rotate-6 transition-transform duration-300"><MonitorCheck size={800} className="text-white" /></div>
                ...
                <header className="flex flex-col md:flex-row items-center justify-between mb-24 relative z-10 border-b border-white/5 pb-16 gap-12">
                   <div className="flex items-center gap-12">
                      ...
                      <div>
                        ...
                        <p className="text-[var(--text-dim)] text-[16px] font-black uppercase tracking-[0.6em] italic leading-none">({content.length}_NODES_STABILIZED_IN_CURRENT_LATTICE)</p>
                      </div>
                   </div>
                   <div className="hidden xl:flex flex-col items-end gap-4 text-[12px] font-black text-[var(--text-dim)] uppercase tracking-[0.5em] italic leading-none opacity-40 hover:opacity-100 transition-opacity">
                      ...
                      >
                          ...
                          <div className="space-y-10 mb-20 relative z-10">
                             {item.folderId && (
                               <div className="flex items-center gap-6 px-8 py-4 bg-white/[0.02] rounded-[2.2rem] border-2 border-white/5 w-fit shadow-2xl group-hover:border-indigo-500/20 transition-all duration-300 group-hover:translate-x-4">
                                  ...
                                  <span className="text-[12px] font-black text-[var(--text-dim)] uppercase tracking-[0.5em] italic truncate max-w-[180px]">{item.folderId.name.toUpperCase()}</span>
                               </div>
                             )}

                             <div className="flex flex-wrap gap-5">
                                {item.tags.slice(0, 3).map((t) => (
                                  <span key={t} className="text-[11px] font-black text-[var(--text-dim)] uppercase tracking-[0.4em] italic leading-none group-hover:text-indigo-400 transition-colors duration-300">#{t.toUpperCase()}</span>
                                ))}
                        ...
                        <div className="flex gap-8 pt-12 border-t border-white/5 mt-auto relative z-10">
                          ...
                          <button onClick={() => handleMatrixClone(item._id)}
                            className="w-24 h-24 bg-white/[0.02] border border-white/10 text-[var(--text-dim)] hover:text-white hover:bg-white/10 flex items-center justify-center rounded-[3.5rem] transition-all duration-300 group/clone shadow-2xl hover:scale-110 active:scale-90" title="Clone Matrix">
                            <Copy size={36} className="group-hover/clone:rotate-12 transition-transform duration-300" />
                          </button>
                        </div>
                      </motion.article>
                    ))}
                ...
                {content.length === 0 && (
                  <div className="flex-1 flex flex-col items-center justify-center text-center p-32 opacity-10 gap-16 group/empty">
                     <div className="w-64 h-64 bg-white/5 rounded-[6rem] border-4 border-white/5 flex items-center justify-center shadow-inner relative group-hover/empty:scale-110 transition-transform duration-300 animate-pulse">
                     ...
                     <div className="space-y-10 max-w-4xl">
                        ...
                        <p className="text-2xl font-black text-[var(--text-dim)] uppercase tracking-[0.8em] italic leading-relaxed">No Payload Matrix detected in current sector.<br/>Initialize an archival capsule to establish permanent storage.</p>
                     </div>
                  </div>
                )}
             </div>
                ...
                <div className={`${glassStyle} rounded-[6rem] p-12 flex items-center justify-between border-indigo-500/60 shadow-[0_100px_250px_rgba(0,0,0,1)] bg-black/90 backdrop-blur-3xl ring-[12px] ring-black/40`}>
                   ...
                   <div className="flex items-center gap-10 flex-1 px-16 justify-center">
                      {[
                        ...
                        <button key={op.title} onClick={op.action} className="w-24 h-24 bg-white/5 border-2 border-white/10 text-[var(--text-dim)] hover:text-white hover:bg-white/10 rounded-[3.5rem] flex items-center justify-center transition-all duration-300 hover:scale-110 shadow-2xl group/op border-none" title={op.title}>
                      ...
                      <button 
                         ...
                         disabled={busy}
                         className="px-20 py-10 bg-rose-600 text-white rounded-[4rem] text-[18px] font-black uppercase tracking-[0.6em] hover:bg-rose-500 transition-all duration-300 shadow-[0_40px_100px_rgba(225,29,72,0.4)] italic flex items-center gap-8 border-none scale-110 hover:scale-120 active:scale-95 group/purge"
                      >
                   ...
                   <button onClick={() => setSelectedItems([])} className="w-20 h-20 bg-white/5 border-2 border-white/10 text-[var(--text-dim)] hover:text-white rounded-[3rem] transition-all mr-12 hover:scale-110 shadow-2xl hover:bg-rose-500/20 active:scale-90 flex items-center justify-center border-none"><X size={44}/></button>
                </div>
             </motion.div>
           )}
                ...
                <div className={`${glassStyle} rounded-[10rem] p-32 max-w-5xl w-full border-white/30 relative overflow-hidden shadow-[0_0_500px_rgba(0,0,0,1)] ring-2 ring-white/10`} onClick={e => e.stopPropagation()}>
                   ...
                   <div className="flex items-center gap-16 mb-32 relative z-10 border-b border-white/5 pb-20">
                      ...
                      <div>
                         ...
                         <p className="text-[16px] font-black text-[var(--text-dim)] uppercase tracking-[0.8em] italic leading-none">Establishing isolated archival sector v12.8.4_PRIME</p>
                      </div>
                   </div>
                   <div className="space-y-24 relative z-10">
                      <div className="space-y-12">
                         <label className="text-[18px] font-black text-[var(--text-dim)] uppercase tracking-[1em] italic leading-none ml-10">Capsule Designation Identity</label>
                         <input type="text" value={newFolderName} onChange={(e) => setNewFolderName(e.target.value)}
                           className="w-full bg-black/80 border-4 border-white/5 rounded-[6rem] px-24 py-16 text-6xl font-black text-white uppercase tracking-tighter focus:outline-none focus:border-indigo-500/50 transition-all italic shadow-inner text-center placeholder:text-[var(--text-dim)] font-mono"
                           placeholder="OMEGA_ARCHIVE_NODE..." autoFocus 
                         />
                      </div>
                      <div className="flex gap-16 items-center pt-16">
                         <button onClick={() => setShowCreateFolder(false)} className="px-16 py-8 text-[18px] font-black text-[var(--text-dim)] uppercase tracking-[0.6em] hover:text-white transition-all italic border-2 border-white/5 rounded-[3.5rem] hover:bg-white/5 border-none">Abort_Sync</button>
                         <button onClick={handleInitCapsule} className="flex-1 py-16 bg-white text-black rounded-[6rem] text-[24px] font-black uppercase tracking-[0.8em] shadow-[0_60px_150px_rgba(255,255,255,0.2)] hover:bg-indigo-500 hover:text-white transition-all duration-300 italic scale-105 active:scale-95 border-none group">
                            <Box className="inline mr-8 group-hover:rotate-180 transition-transform duration-300" size={40} /> Establish Archival Capsule
                         </button>
                      </div>
                   </div>
                </div>
               ...
               <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[200] flex items-center justify-center p-12 bg-black/98 backdrop-blur-3xl" onClick={() => setTagModalIds(null)}>
                  <div className={`${glassStyle} rounded-[10rem] p-32 max-w-5xl w-full border-white/30 relative overflow-hidden shadow-2xl`} onClick={e => e.stopPropagation()}>
                     ...
                     <input type="text" value={tagInput} onChange={(e) => setTagInput(e.target.value)} placeholder="ENTER_METADATA_SEQUENCE..."
                       className="w-full bg-black/80 border-4 border-white/5 rounded-[6rem] px-24 py-20 text-6xl font-black text-white uppercase tracking-tighter focus:outline-none focus:border-indigo-500/50 transition-all italic shadow-inner mb-32 text-center placeholder:text-[var(--text-dim)] font-mono" 
                     />
                     <div className="flex gap-16 items-center">
                        <button onClick={() => setTagModalIds(null)} className="px-16 py-8 text-[18px] font-black text-[var(--text-dim)] uppercase tracking-[0.6em] hover:text-white transition-all italic border-none">ABORT_INJECTION</button>
                        <button onClick={handleTagInjection} disabled={busy} className="flex-1 py-16 bg-white text-black rounded-[6rem] text-[24px] font-black uppercase tracking-[0.8em] shadow-2xl hover:bg-indigo-500 hover:text-white transition-all duration-300 italic scale-105 active:scale-95 border-none group">
           ...
           {moveModalIds && (
               <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[200] flex items-center justify-center p-12 bg-black/98 backdrop-blur-3xl" onClick={() => setMoveModalIds(null)}>
                  <div className={`${glassStyle} rounded-[10rem] p-32 max-w-5xl w-full border-white/30 relative overflow-hidden shadow-2xl`} onClick={e => e.stopPropagation()}>
                     ...
                     <div className="flex gap-16 items-center">
                        <button onClick={() => setMoveModalIds(null)} className="px-16 py-8 text-[18px] font-black text-[var(--text-dim)] uppercase tracking-[0.6em] hover:text-white transition-all italic border-none">ABORT_SHIFT</button>
                        <button onClick={handleLatticeTransfer} disabled={busy} className="flex-1 py-16 bg-white text-black rounded-[6rem] text-[24px] font-black uppercase tracking-[0.8em] shadow-2xl hover:bg-indigo-500 hover:text-white transition-all duration-300 italic scale-105 active:scale-95 border-none group">
           ...
           {exportModalIds && (
               <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[200] flex items-center justify-center p-12 bg-black/98 backdrop-blur-3xl" onClick={() => setExportModalIds(null)}>
                  <div className={`${glassStyle} rounded-[10rem] p-32 max-w-5xl w-full border-white/30 relative overflow-hidden shadow-2xl`} onClick={e => e.stopPropagation()}>
                     ...
                     <div className="flex gap-16 items-center">
                        <button onClick={() => setExportModalIds(null)} className="px-16 py-8 text-[18px] font-black text-[var(--text-dim)] uppercase tracking-[0.6em] hover:text-white transition-all italic border-none">ABORT_EXTRACTION</button>
                        <button onClick={handleExtraction} disabled={busy} className="flex-1 py-16 bg-white text-black rounded-[6rem] text-[24px] font-black uppercase tracking-[0.8em] shadow-2xl hover:bg-indigo-500 hover:text-white transition-all duration-300 italic scale-105 active:scale-95 border-none group">
