                   ...
                   <div className="space-y-6">
                     ...
                     {folders.map((folder) => (
                       <button key={folder._id} onClick={() => setSelectedFolder(folder._id)}
                         className={`w-full group text-left px-10 py-6 rounded-[2.5rem] text-[13px] font-black uppercase tracking-[0.4em] transition-all duration-300 flex items-center gap-8 italic ${selectedFolder === folder._id ? 'bg-white text-black shadow-2xl scale-105' : 'text-slate-400 hover:text-white hover:bg-white/[0.04]'}`}
                       >
                   ...
                   <div className="flex flex-wrap gap-4">
                     {tags.map((tag) => (
                       <button key={tag} onClick={() => setSelectedTag(selectedTag === tag ? null : tag)}
                         className={`px-8 py-4 rounded-[2rem] text-[12px] font-black uppercase tracking-[0.2em] transition-all duration-300 border-2 italic ${selectedTag === tag ? 'bg-indigo-600 text-white border-indigo-400 shadow-[0_0_40px_rgba(99,102,241,0.5)]' : 'bg-black/40 text-slate-400 border-white/5 hover:border-white/20 hover:text-white'}`}
                       >
                         #{tag.toUpperCase()}
                       </button>
                     ))}
          ...
          <main className="lg:col-span-3 space-y-16">
             <div className={`${glassStyle} p-20 rounded-[7rem] min-h-[1100px] border-white/5 relative overflow-hidden shadow-[0_80px_200px_rgba(0,0,0,0.8)] flex flex-col bg-black/40`}>
                ...
                <header className="flex flex-col md:flex-row items-center justify-between mb-24 relative z-10 border-b border-white/5 pb-16 gap-12">
                   ...
                   <div className="hidden xl:flex flex-col items-end gap-4 text-[12px] font-black text-slate-500 uppercase tracking-[0.5em] italic leading-none opacity-40 hover:opacity-100 transition-opacity">
                      ...
                      >
                        ...
                        <div className="flex gap-8 pt-12 border-t border-white/5 mt-auto relative z-10">
                          ...
                          <button onClick={() => handleMatrixClone(item._id)}
                            className="w-24 h-24 bg-white/[0.02] border border-white/10 text-slate-500 hover:text-white hover:bg-white/10 flex items-center justify-center rounded-[3.5rem] transition-all duration-300 group/clone shadow-2xl hover:scale-110 active:scale-90" title="Clone Matrix">
                ...
                <div className={`${glassStyle} rounded-[6rem] p-12 flex items-center justify-between border-indigo-500/60 shadow-[0_100px_250px_rgba(0,0,0,1)] bg-black/90 backdrop-blur-3xl ring-[12px] ring-black/40`}>
                   ...
                   <div className="flex items-center gap-10 flex-1 px-16 justify-center">
                      {[
                        ...
                        <button key={op.title} onClick={op.action} className="w-24 h-24 bg-white/5 border-2 border-white/10 text-slate-500 hover:text-white hover:bg-white/10 rounded-[3.5rem] flex items-center justify-center transition-all duration-300 hover:scale-110 shadow-2xl group/op border-none" title={op.title}>
             ...
             <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[200] flex items-center justify-center p-12 bg-black/98 backdrop-blur-3xl" onClick={() => setShowCreateFolder(false)}>
                <div className={`${glassStyle} rounded-[10rem] p-32 max-w-5xl w-full border-white/30 relative overflow-hidden shadow-[0_0_500px_rgba(0,0,0,1)] ring-2 ring-white/10`} onClick={e => e.stopPropagation()}>
                   ...
                   <div className="space-y-24 relative z-10">
                      <div className="space-y-12">
                         <label className="text-[18px] font-black text-slate-400 uppercase tracking-[1em] italic leading-none ml-10">Capsule Designation Identity</label>
                         <input type="text" value={newFolderName} onChange={(e) => setNewFolderName(e.target.value)}
                           className="w-full bg-black/80 border-4 border-white/5 rounded-[6rem] px-24 py-16 text-6xl font-black text-white uppercase tracking-tighter focus:outline-none focus:border-indigo-500/50 transition-all italic shadow-inner text-center placeholder:text-slate-600 font-mono"
                           placeholder="OMEGA_ARCHIVE_NODE..." autoFocus 
                         />
                      </div>
                      <div className="flex gap-16 items-center pt-16">
                         <button onClick={() => setShowCreateFolder(false)} className="px-16 py-8 text-[18px] font-black text-slate-400 uppercase tracking-[0.6em] hover:text-white transition-all italic border-2 border-white/5 rounded-[3.5rem] hover:bg-white/5 border-none">Abort_Sync</button>
               ...
               <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[200] flex items-center justify-center p-12 bg-black/98 backdrop-blur-3xl" onClick={() => setTagModalIds(null)}>
                  <div className={`${glassStyle} rounded-[10rem] p-32 max-w-5xl w-full border-white/30 relative overflow-hidden shadow-2xl`} onClick={e => e.stopPropagation()}>
                     ...
                     <div className="flex gap-16 items-center">
                        <button onClick={() => setTagModalIds(null)} className="px-16 py-8 text-[18px] font-black text-slate-400 uppercase tracking-[0.6em] hover:text-white transition-all italic border-none">ABORT_INJECTION</button>
           ...
           {moveModalIds && (
               <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[200] flex items-center justify-center p-12 bg-black/98 backdrop-blur-3xl" onClick={() => setMoveModalIds(null)}>
                  <div className={`${glassStyle} rounded-[10rem] p-32 max-w-5xl w-full border-white/30 relative overflow-hidden shadow-2xl`} onClick={e => e.stopPropagation()}>
                     ...
                     <div className="flex gap-16 items-center">
                        <button onClick={() => setMoveModalIds(null)} className="px-16 py-8 text-[18px] font-black text-slate-400 uppercase tracking-[0.6em] hover:text-white transition-all italic border-none">ABORT_SHIFT</button>
           ...
           {exportModalIds && (
               <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[200] flex items-center justify-center p-12 bg-black/98 backdrop-blur-3xl" onClick={() => setExportModalIds(null)}>
                  <div className={`${glassStyle} rounded-[10rem] p-32 max-w-5xl w-full border-white/30 relative overflow-hidden shadow-2xl`} onClick={e => e.stopPropagation()}>
                     ...
                     <div className="flex gap-16 items-center">
                        <button onClick={() => setExportModalIds(null)} className="px-16 py-8 text-[18px] font-black text-slate-400 uppercase tracking-[0.6em] hover:text-white transition-all italic border-none">ABORT_EXTRACTION</button>
