      ...
      <div className="min-h-screen relative z-10 pb-48 px-10 pt-16 max-w-[1700px] mx-auto space-y-24">
        ...
        <div className="flex flex-col lg:flex-row items-center justify-between gap-16 relative z-50">
           ...
           <button 
             ...
             className="px-16 py-8 bg-white text-black rounded-[3rem] text-[15px] font-black uppercase tracking-[0.5em] shadow-[0_50px_150px_rgba(255,255,255,0.15)] hover:bg-indigo-500 hover:text-white transition-all duration-300 flex items-center gap-8 italic active:scale-95 group"
           >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-16 relative z-10">
           {workflows.map((w) => (
             <motion.div 
               ...
               className={`${glassStyle} rounded-[5rem] p-16 flex flex-col hover:border-indigo-500/50 hover:bg-white/[0.05] transition-all duration-300 group shadow-[0_100px_300px_rgba(0,0,0,0.8)] relative overflow-hidden`}
             >
                ...
                <div className="flex gap-6 relative z-10">
                   <button 
                     ...
                     className="flex-1 py-6 bg-white text-black rounded-[2.5rem] text-[15px] font-black uppercase tracking-[0.5em] hover:bg-indigo-500 hover:text-white transition-all duration-300 flex items-center justify-center gap-6 shadow-3xl active:scale-95 disabled:opacity-20 italic group/btn"
                   >
                   ...
                   <button 
                     ...
                     className="w-20 h-20 rounded-[2.2rem] bg-white/[0.03] border-2 border-white/10 flex items-center justify-center text-slate-500 hover:text-white transition-all duration-300 shadow-3xl active:scale-95 hover:border-indigo-500/50 hover:bg-white/[0.05] group"
                   >
                ...
                <motion.div initial={{ opacity: 0, scale: 0.9, y: 100 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 100 }} className={`${glassStyle} rounded-[6rem] p-24 max-w-6xl w-full border-white/20 max-h-[95vh] overflow-y-auto custom-scrollbar relative shadow-[0_150px_400px_rgba(0,0,0,1)]`} onClick={e => e.stopPropagation()}>
                   ...
                   <div className="flex justify-between items-center mb-20 relative z-10 border-b-2 border-white/10 pb-12">
                      ...
                      <button 
                        ...
                        title="Close Modal"
                        className="w-20 h-20 rounded-[2.5rem] bg-white/[0.03] border-2 border-white/10 flex items-center justify-center text-slate-500 hover:text-white transition-all hover:rotate-90 hover:border-rose-500/50"
                      >
                        <X size={40} />
                      </button>
                   </div>
                   ...
                   <div className="space-y-20 relative z-10">
                      <div className="space-y-8">
                        <label className="text-[14px] font-black text-slate-500 uppercase tracking-[0.8em] italic leading-none ml-8">SEQUENCE_DESIGNATION</label>
                        <input 
                          value={form.name} 
                          onChange={e => setForm(f => ({ ...f, name: e.target.value }))} 
                          className="w-full bg-black/60 border-2 border-white/10 rounded-[3.5rem] px-16 py-12 text-6xl font-black text-white italic uppercase focus:border-indigo-500/50 transition-all shadow-inner placeholder:text-slate-600" 
                      ...
                      <div className="space-y-8">
                        <label className="text-[14px] font-black text-slate-500 uppercase tracking-[0.8em] italic leading-none ml-8">OPERATIONAL_PARAMETERS</label>
                        <textarea 
                          value={form.description} 
                          onChange={e => setForm(f => ({ ...f, description: e.target.value }))} 
                          rows={4} 
                          className="w-full bg-black/60 border-2 border-white/10 rounded-[4.5rem] px-16 py-16 text-2xl font-black text-slate-400 italic uppercase focus:border-indigo-500/50 transition-all placeholder:text-slate-600 leading-relaxed shadow-inner" 
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-16">
                         <div className="space-y-8">
                           <label className="text-[14px] font-black text-slate-500 uppercase tracking-[0.8em] italic leading-none ml-8">SQUADRON_ASSIGNMENT</label>
                           <div className="relative">
                             <select 
                               value={form.teamId} 
                         ...
                         <div className="space-y-8">
                           <label className="text-[14px] font-black text-slate-500 uppercase tracking-[0.8em] italic leading-none ml-8">METADATA_LATTICE_TAGS</label>
                           <input 
                             value={form.tags} 
                             onChange={e => setForm(f => ({ ...f, tags: e.target.value }))} 
                             className="w-full bg-black/60 border-2 border-white/10 rounded-[3rem] px-12 py-10 text-[16px] font-black text-white uppercase tracking-[0.5em] italic shadow-inner placeholder:text-slate-600" 
                      ...
                      <div className="flex flex-col sm:flex-row items-center justify-between gap-16 pt-20 border-t-2 border-white/10">
                         <button onClick={() => setShowCreateModal(false)} className="text-[16px] font-black text-rose-500/20 hover:text-rose-500 uppercase tracking-[0.8em] transition-all duration-300 italic">ABORT_SYNTHESIS</button>
                         <button onClick={handleSave} className="px-32 py-12 bg-white text-black rounded-[4rem] text-[20px] font-black uppercase tracking-[0.8em] shadow-[0_50px_100px_rgba(255,255,255,0.15)] hover:bg-indigo-500 hover:text-white transition-all duration-300 italic active:scale-95">SEAL_SEQUENCE_PARAMETERS</button>
                      </div>
                   </div>
                </motion.div>
             </div>
