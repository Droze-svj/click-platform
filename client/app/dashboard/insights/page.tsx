      ...
      <div className="min-h-screen relative z-10 pb-24 md:pb-48 px-4 md:px-6 lg:px-10 pt-8 md:pt-12 lg:pt-16 max-w-[1750px] mx-auto space-y-12 md:space-y-24">
        ...
        <header className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-16 relative z-50">
           <div className="flex items-center gap-12">
              ...
              <div className="w-24 h-24 bg-cyan-500/5 border border-[var(--tint-cyan-edge)] rounded-[3rem] flex items-center justify-center shadow-2xl relative group overflow-hidden">
                <div className="absolute inset-0 bg-gradient-to-br from-cyan-500/20 to-transparent opacity-100" />
                <Cpu size={44} className="text-[var(--tint-cyan-fg)] relative z-10 group-hover:scale-125 transition-transform duration-300" />
              </div>
              <div>
                 <div className="flex items-center gap-6 mb-3">
                   <div className="flex items-center gap-3">
                      <Shield size={16} className="text-[var(--tint-cyan-fg)] animate-pulse" />
                      <span className="text-[12px] font-black uppercase tracking-[0.6em] text-[var(--tint-cyan-fg)] italic leading-none">Cognitive Matrix v14.2</span>
                   </div>
                   <div className="flex items-center gap-3 px-4 py-1.5 rounded-full bg-black/40 border border-white/5 shadow-inner">
                       <ActivitySquare size={12} className="text-[var(--tint-cyan-fg)] animate-pulse" />
        ...
        <AnimatePresence>
          {error && (
            ...
            >
               <div className="p-12 rounded-[4rem] bg-[var(--tint-rose-bg)] border border-[var(--tint-rose-edge)] flex items-center justify-between gap-8 group shadow-2xl">
                  <div className="flex items-center gap-10">
                     <div className="w-16 h-16 bg-[var(--tint-rose-bg)] rounded-3xl flex items-center justify-center border border-[var(--tint-rose-edge)]"><AlertCircle className="text-[var(--tint-rose-fg)]" size={32} /></div>
                     <p className="text-2xl font-black text-[var(--tint-rose-fg)] uppercase tracking-tighter italic leading-none">{error}</p>
                  </div>
        ...
        <div className={`${glassStyle} rounded-[6rem] p-16 relative z-10 group overflow-hidden border-cyan-500/10 hover:border-[var(--tint-cyan-edge)] shadow-[inset_0_0_100px_rgba(0,0,0,0.4)]`}>
          ...
          <div className="flex flex-col xl:flex-row items-center justify-between gap-12 relative z-10">
            <div className="flex items-center gap-12">
              <div className="w-24 h-24 bg-cyan-600/5 border border-cyan-500/10 rounded-[3rem] flex items-center justify-center shadow-2xl group-hover:rotate-12 transition-transform duration-300">
                <Link2 className="w-12 h-12 text-[var(--tint-cyan-fg)]" />
              </div>
              <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-12 relative z-10">
          <HeuristicStatCard icon={Globe} label="Spectral Reach" value={audienceInsights?.insights?.overview?.totalImpressions ?? platformAudience?.totalReach ?? 0} format="number" color="text-[var(--tint-indigo-fg)]" />
          <HeuristicStatCard icon={Activity} label="Signal Affinity" value={audienceInsights?.insights?.overview?.totalEngagement ?? platformAudience?.totalEngagement ?? 0} format="number" color="text-[var(--tint-emerald-fg)]" />
          <HeuristicStatCard icon={Zap} label="Resonance Rate" value={audienceInsights?.insights?.overview?.engagementRate ?? platformAudience?.averageEngagementRate ?? 0} format="percent" color="text-[var(--tint-amber-fg)]" />
          <HeuristicStatCard icon={Target} label="Primary Vector" value={platformAudience?.topPlatform ?? 'N/A'} format="text" color="text-[var(--tint-rose-fg)]" />
        </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-20 relative z-10">
          ...
          <div className={`${glassStyle} rounded-[6rem] overflow-hidden group border-indigo-500/10 shadow-[0_60px_150px_rgba(0,0,0,0.6)]`}>
            <div className="px-16 py-12 border-b border-white/5 flex items-center gap-10 bg-white/[0.01]">
              <div className="w-16 h-16 rounded-[2rem] bg-[var(--tint-indigo-bg)] flex items-center justify-center border border-[var(--tint-indigo-edge)] shadow-2xl transition-all group-hover:rotate-12 group-hover:scale-110"><Users size={32} className="text-[var(--tint-indigo-fg)]" /></div>
                ...
                <div className="space-y-16">
                  {audienceInsights?.insights?.overview?.platformDistribution && (
                    <div className="space-y-12">
                      <p className="text-[12px] font-black text-slate-400 uppercase tracking-[0.8em] italic leading-none pl-6 border-l-2 border-[var(--tint-indigo-edge)]">Density Distribution Matrix</p>
                      <div className="grid grid-cols-1 gap-6">
                        {Object.entries(audienceInsights.insights.overview.platformDistribution).map(([p, count], idx) => (
                          <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.1 }}
                            key={p} className="p-10 bg-black/40 border border-white/5 rounded-[3.5rem] flex items-center justify-between group/node hover:bg-indigo-500/5 hover:border-[var(--tint-indigo-edge)] transition-all duration-700 shadow-[inset_0_0_50px_rgba(0,0,0,0.4)]"
                          >
                            <span className="text-3xl font-black text-white italic tracking-tighter uppercase leading-none group-hover:text-[var(--tint-indigo-fg)] transition-colors">{p}</span>
                            <div className="flex items-center gap-10">
                               ...
                               <span className="text-2xl font-black text-[var(--tint-indigo-fg)] italic tabular-nums leading-none tracking-tighter">{String(count).toUpperCase()} NODES</span>
                            </div>
                          </motion.div>
                        ))}
                      </div>
                  ...
                  {platformAudience?.bestPostingTime && (
                    <div className="flex items-center gap-12 p-12 bg-cyan-500/5 border border-cyan-500/10 rounded-[4.5rem] shadow-2xl group/opt transition-all duration-300 hover:bg-[var(--tint-cyan-bg)] relative overflow-hidden">
                      ...
                      <div className="w-24 h-24 bg-cyan-600/10 border border-[var(--tint-cyan-edge)] text-[var(--tint-cyan-fg)] rounded-[2.5rem] flex items-center justify-center shadow-2xl group-hover/opt:rotate-12 transition-transform duration-700"><Calendar size={48} /></div>
                      <div className="relative z-10">
                          <p className="text-[12px] font-black text-[var(--tint-cyan-fg)]/60 uppercase tracking-[0.6em] italic mb-4 leading-none truncate">Spectral_Window_Optimal</p>
          ...
          <div className={`${glassStyle} rounded-[6rem] overflow-hidden group border-amber-500/10 shadow-[0_60px_150px_rgba(0,0,0,0.6)]`}>
            <div className="px-16 py-12 border-b border-white/5 flex items-center gap-10 bg-white/[0.01]">
              <div className="w-16 h-16 rounded-[2rem] bg-[var(--tint-amber-bg)] flex items-center justify-center border border-[var(--tint-amber-edge)] shadow-2xl transition-all group-hover:rotate-[-12deg] group-hover:scale-110"><Sparkles size={32} className="text-[var(--tint-amber-fg)]" /></div>
                ...
                <div className="grid grid-cols-1 gap-8">
                  {audienceInsights.insights.recommendations.slice(0, 5).map((rec, i) => (
                    <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.1 }} key={i}
                      className="flex items-center gap-12 p-10 bg-black/40 border border-amber-500/5 rounded-[4rem] group/rec hover:bg-amber-500/5 hover:border-[var(--tint-amber-edge)] transition-all duration-700 shadow-inner"
                    >
                      <div className="w-16 h-16 bg-[var(--tint-amber-bg)] border border-[var(--tint-amber-edge)] rounded-[2rem] flex items-center justify-center text-amber-500 flex-shrink-0 group-hover/rec:scale-110 group-hover/rec:rotate-12 transition-all duration-700 shadow-2xl">
                         <Zap size={32} />
                      </div>
                      <div>
                          <div className="flex items-center gap-6 mb-3">
                             <span className="text-[12px] font-black text-[var(--tint-amber-fg)] uppercase tracking-widest italic leading-none">SYNTHETIC_PACKET_V{i+1}</span>
                             <div className="h-0.5 flex-1 bg-[var(--tint-amber-bg)]" />
                          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-16 relative z-10 pt-16">
          {[
            { label: 'Spectral Sync', desc: 'Detailed performance diagnostics.', icon: BarChart3, href: '/dashboard/analytics', color: 'text-[var(--tint-indigo-fg)]' },
...
function HeuristicStatCard({ icon: Icon, label, value, format, color }: { icon: any; label: string; value: any; format: 'number' | 'percent' | 'text'; color: string }) {
  ...
  return (
    ...
    >
      ...
      <div className={`text-6xl font-black italic tabular-nums leading-none tracking-tighter mb-6 text-white group-hover:text-[var(--tint-cyan-fg)] transition-colors duration-700 drop-shadow-2xl`}>{display.toUpperCase()}</div>
      <div className="text-[14px] text-slate-400 font-black uppercase tracking-[0.4em] italic leading-none">{label}</div>
      <div className="text-[10px] text-slate-500 font-black uppercase tracking-widest mt-8 italic bg-white/5 px-6 py-2 rounded-full border border-white/5 group-hover:text-[var(--tint-cyan-fg)] transition-colors">HEURISTIC_SIGNAL</div>
    </motion.div>
  )
}
