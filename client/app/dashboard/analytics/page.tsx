...
export default function SovereignAnalyticsMatrix() {
  ...
  if (loading) return (
     <div className="min-h-screen bg-surface-page dark:bg-black transition-colors duration-500 px-4 sm:px-6 lg:px-12 pt-8 max-w-[1900px] mx-auto" aria-busy="true" aria-label={t('analyticsPage.loading')}>
      ...
      <div className="min-h-screen relative z-10 pb-48 px-4 sm:px-6 lg:px-12 pt-8 max-w-[1900px] mx-auto space-y-12 bg-transparent text-white transition-colors duration-500 font-inter">
        ...
        <header className="flex flex-col lg:flex-row justify-between items-center gap-12 pb-10 border-b-2 border-white/5 relative z-50">
           <div className="flex items-center gap-6 w-full lg:w-auto min-w-0">
              <button type="button" onClick={() => router.push('/dashboard')} title={t('analyticsPage.backToDashboard')} aria-label={t('analyticsPage.backToDashboard')} className="w-14 h-14 rounded-2xl bg-black/40 border-2 border-white/10 flex items-center justify-center text-slate-400 hover:text-white transition-all shadow-sm active:scale-90 group">
              ...
              <div className="flex-1 min-w-0">
                 <div className="flex items-center gap-4 mb-2 flex-wrap">
                    <span className="px-3 py-1 rounded-lg text-[10px] font-black bg-primary-900/30 text-primary-400 uppercase tracking-[0.1em] sm:tracking-[0.2em] border border-primary-500/20 italic leading-none">
                      {t('analyticsPage.badgeDiagnostics')}
                    </span>
                    <div className="flex items-center gap-2 px-3 py-1 rounded-lg bg-black/40 text-slate-500 border border-white/10 text-[10px] font-black italic shadow-inner">
                        ...
                        {t('analyticsPage.badgeLive')}
                    </div>
                 </div>
                 <h1 className="text-3xl sm:text-5xl font-black tracking-tighter leading-none mt-3 truncate uppercase italic text-white">{t('analyticsPage.title')}</h1>
              </div>
           </div>
           ...
           <div className="flex flex-wrap items-center gap-6 justify-end w-full lg:w-auto">
              <button
                ...
                title={t('analyticsPage.openAiInsights')}
                aria-label={t('analyticsPage.openAiInsights')}
              >
              ...
              <button
                ...
                onClick={fetchMatrix}
                disabled={refreshing}
                title={t('analyticsPage.synchronize')}
                aria-label={t('analyticsPage.synchronize')}
        ...
        {fetchError && (
          ...
          >
            ...
            <div className="flex-1 min-w-0 text-center md:text-left">
              <p className="text-[11px] font-black uppercase tracking-[0.5em] text-rose-500 italic mb-3">{t('analyticsPage.errorEyebrow')}</p>
              <h3 className="text-2xl font-black text-white tracking-tighter leading-tight mb-2 uppercase italic">{t('analyticsPage.errorTitle')}</h3>
              <p className="text-sm font-bold text-slate-500 italic">{t('analyticsPage.errorSubtitle')}</p>
            </div>
            <button type="button" onClick={fetchMatrix} title={t('analyticsPage.retry')} aria-label={t('analyticsPage.retryAnalyticsFetch')}
        ...
        {data?.isFallback && (
          ...
          >
            ...
            <div className="flex-1 min-w-0 text-center md:text-left">
              <p className="text-[10px] sm:text-[11px] font-black uppercase tracking-[0.5em] text-amber-500 italic mb-3">{t('analyticsPage.emptyEyebrow')}</p>
              <h3 className="text-2xl sm:text-3xl font-black text-white tracking-tighter leading-tight mb-3 uppercase italic group-hover:text-amber-500 transition-colors">{t('analyticsPage.emptyTitle')}</h3>
               <p className="text-sm font-bold text-slate-500 italic leading-relaxed uppercase tracking-tight">
                ...
                <Link href="/dashboard/social" className="text-amber-500 underline hover:no-underline font-black hover:text-white transition-colors">{t('analyticsPage.connectAccounts')}</Link>{' '}
                {t('analyticsPage.emptyBodyAfter')}
              </p>
            </div>
            <button type="button" onClick={() => router.push('/dashboard/social')} title={t('analyticsPage.connectPlatform')} aria-label={t('analyticsPage.connectPlatform')} className="px-8 py-4 bg-white text-black rounded-2xl text-[11px] font-black uppercase tracking-[0.4em] italic shadow-2xl active:scale-95 border-none hover:bg-amber-500 hover:text-white transition-all">{t('analyticsPage.connect')}</button>
          </motion.div>
        )}
          ...
          >
            ...
            <div className="flex flex-col items-center gap-10 shrink-0 relative z-10">
              <div className="w-56 h-56 sm:w-64 sm:h-64 rounded-full border-[10px] sm:border-[14px] border-primary-500/10 flex flex-col items-center justify-center bg-black/40 shadow-[inset_0_0_80px_rgba(0,0,0,0.8)] relative group/ring"
                aria-label={t('analyticsPage.avgAffinityRate', { rate: data?.avg_engagement_rate || 0 })}
              >
                ...
                <span className="text-[10px] sm:text-[12px] font-black text-primary-500 uppercase tracking-[0.4em] sm:tracking-[0.6em] mt-3 sm:mt-4 italic drop-shadow-[0_0_8px_rgba(99,102,241,0.5)]">{t('analyticsPage.engagement')}</span>
              </div>
               <div className="px-8 py-3 rounded-2xl bg-black/40 border-2 border-white/10 shadow-2xl transition-all group-hover:border-primary-500/30">
                  <span className="text-[11px] font-black text-slate-700 uppercase tracking-[0.5em] italic leading-none">{t('analyticsPage.engagementScore')}</span>
               </div>
            </div>
            ...
            <div className="space-y-10 flex-1 relative z-10 text-center lg:text-left">
              <div className="flex items-center gap-4 justify-center lg:justify-start flex-wrap">
                 <Sparkles className="text-primary-500 animate-pulse" size={24} />
                  <span className="text-[12px] font-black text-primary-500 uppercase tracking-[0.4em] italic drop-shadow-[0_0_8px_rgba(99,102,241,0.5)]" title={t('analyticsPage.aiInsight')} aria-label={t('analyticsPage.aiInsight')}>{t('analyticsPage.aiInsight')}</span>
                  ...
                  <button
                    ...
                    onClick={() => fetchMatrix()}
                    disabled={refreshing}
                    title={t('analyticsPage.refreshInsight')}
                  ...
                  >
                    ...
                    {refreshing ? t('analyticsPage.refreshing') : t('analyticsPage.refresh')}
                  </button>
              </div>
              ...
              <h2 className="text-2xl sm:text-4xl lg:text-5xl font-black italic text-white leading-tight uppercase tracking-tight max-w-3xl min-w-0 [overflow-wrap:anywhere]">
                  &ldquo;{data?.ai_summary?.headline || t('analyticsPage.aiInsightPlaceholder')}&rdquo;
              </h2>
               <div className="flex flex-wrap gap-6 justify-center lg:justify-start">
                  <div className="flex items-center gap-4 px-6 py-3 rounded-2xl bg-emerald-500/10 border-2 border-emerald-500/20 text-[10px] font-black text-emerald-400 uppercase tracking-widest italic shadow-xl backdrop-blur-xl">
                     <CheckCircle2 size={16} /> {t('analyticsPage.statusHealthy')}
                  </div>
                  <div className="flex items-center gap-4 px-6 py-3 rounded-2xl bg-primary-500/10 border-2 border-primary-500/20 text-[10px] font-black text-primary-400 uppercase tracking-widest italic shadow-xl backdrop-blur-xl">
                     <ActivitySquare size={16} /> {t('analyticsPage.statusEngaged')}
                  </div>
               </div>
            </div>
          </motion.section>
            ...
            <div className="flex flex-col md:flex-row items-center justify-between px-6 sm:px-14 py-12 border-b-2 border-white/5 bg-white/5 gap-10 relative z-10">
               <div className="flex flex-col sm:flex-row items-center gap-6 sm:gap-8 text-white">
                  ...
                  <div className="text-center sm:text-left">
                     <h3 className="text-3xl sm:text-4xl font-black italic uppercase tracking-tighter leading-none mb-2">{t('analyticsPage.recentPosts')}</h3>
                     <p className="text-[9px] sm:text-[11px] text-slate-500 font-black uppercase tracking-[0.3em] sm:tracking-[0.5em] italic leading-none">{t('analyticsPage.recentPostsSubtitle')}</p>
                  </div>
               </div>
              <div className="flex items-center gap-6 w-full md:w-auto">
                 <div className="relative flex-1 min-w-0 md:w-80 group/search">
                    ...
                    <input 
                       ...
                       title={t('analyticsPage.searchPosts')}
                  ...
                  <div className="relative shrink-0">
                    <button
                      ...
                      title={t('analyticsPage.filterByPlatform')}
             ...
             <AnimatePresence>
               {filteredNodes.map((node, idx) => (
                 <motion.div key={node.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} transition={{ delay: idx * 0.05 }}
                    title={t('analyticsPage.viewDetailsFor', { title: node.title })}
                 ...
                 >
                   ...
                   <div className="flex-1 min-w-0">
                     ...
                     <div className="flex items-center gap-8 mt-3 text-[11px] font-black text-slate-700 uppercase tracking-widest italic leading-none">
                        ...
                        <div className="w-2 h-2 rounded-full bg-white/5" />
                        <div className="flex items-center gap-3"><Shield size={16} className="text-emerald-500" /> <span className="text-emerald-500">{t('analyticsPage.badgeLive')}</span></div>
                     </div>
                   </div>
                   ...
                   <div className="flex items-center gap-6 sm:gap-12 shrink-0">
                     <div className="text-right hidden sm:block">
                       <div className="text-2xl sm:text-4xl font-black text-white italic tracking-tighter tabular-nums leading-none mb-1 group-hover:text-primary-500 transition-colors">{fmt(node.views)}</div>
                        <div className="text-[8px] sm:text-[10px] text-slate-700 font-black uppercase tracking-[0.2em] sm:tracking-[0.3em] italic">{t('analyticsPage.views')}</div>
                     </div>
                      <div className={`w-12 h-12 sm:w-16 sm:h-16 rounded-xl flex flex-col items-center justify-center border-2 shrink-0 shadow-xl transition-all group-hover:scale-110 ${(node.viralScore ?? 0) >= 80 ? 'border-emerald-500/30 text-emerald-500 bg-emerald-500/10' : 'border-primary-500/30 text-primary-500 bg-primary-500/10'}`}>
                       ...
                       <span className="text-[6px] sm:text-[8px] font-black mt-0.5 sm:mt-1 uppercase tracking-widest">{t('analyticsPage.score')}</span>
                     </div>
                     <button type="button" onClick={(e) => { e.stopPropagation(); router.push(`/dashboard/analytics/posts/${node.id}`); }} title={t('analyticsPage.viewPostAnalytics')} aria-label={t('analyticsPage.viewPostAnalytics')} className="w-10 h-10 sm:w-12 sm:h-12 rounded-lg sm:rounded-xl bg-black/40 border border-white/10 flex items-center justify-center text-slate-800 hover:text-primary-500 hover:border-primary-500/30 transition-all active:scale-90">
              ...
              {(filteredNodes.length === 0 && !loading) && (
                <div className="py-48 flex flex-col items-center justify-center opacity-10 group-hover:opacity-20 transition-opacity duration-1000">
                   ...
                   <span className="text-3xl font-black uppercase tracking-[0.8em] italic text-slate-900">{t('analyticsPage.noPostsYet')}</span>
                </div>
              )}
           </div>
           <div className="px-10 py-8 bg-black/40 border-t-2 border-white/5 flex items-center justify-between">
              <p className="text-[10px] font-black text-slate-900 uppercase tracking-[0.5em] italic">{t('analyticsPage.secureConnection')}</p>
