...
const PLATFORM_CFG: Record<string, { gradient: string; icon: string; color: string }> = {
  ...
  facebook:  { gradient: 'from-indigo-600 to-indigo-900',   icon: 'f', color: 'text-[var(--tint-indigo-fg)]'  },
...
export default function SpectralIntelligencePage() {
  ...
  if (error || !data) return (
     <div className="min-h-screen relative z-10 pb-24 px-8 pt-12 max-w-[1700px] mx-auto space-y-16 bg-[#020205]">
        ...
        <div className={`${glassStyle} p-32 rounded-[5rem] text-center border-[var(--tint-rose-edge)] max-w-3xl mx-auto shadow-[0_0_200px_rgba(225,29,72,0.1)]`}>
  ...
  const STATS = [
    { label: 'Operational Nodes', value: data.overview.total_posts, sub: `${data.overview.published_posts} ACTIVE_PHANTOMS`, icon: Cpu, color: 'text-[var(--tint-indigo-fg)]' },
    { label: 'Spectral Saturation', value: fmt(data.overview.total_views), sub: 'CUMULATIVE_REACH', icon: Eye, color: 'text-[var(--tint-emerald-fg)]' },
    ...
    { label: 'Trajectory Kinetic', value: data.overview.avg_engagement_rate, sub: 'PEAK_MOMENTUM_HZ', icon: TrendingUp, color: 'text-[var(--tint-amber-fg)]' },
  ]
      ...
      <div className="min-h-screen relative z-10 pb-24 px-10 pt-16 max-w-[1750px] mx-auto space-y-24">
        ...
        <header className="flex flex-col 2xl:flex-row justify-between items-start 2xl:items-center gap-16 relative z-50">
           <div className="flex items-center gap-12">
              ...
              <div className="w-24 h-24 bg-indigo-500/5 border border-[var(--tint-indigo-edge)] rounded-[3rem] flex items-center justify-center shadow-2xl relative group overflow-hidden">
                ...
                <BarChart3 size={44} className="text-[var(--tint-indigo-fg)] relative z-10 group-hover:scale-125 transition-transform duration-700" />
              </div>
              <div>
                 <div className="flex items-center gap-6 mb-3">
                   <div className="flex items-center gap-3">
                      <Fingerprint size={16} className="text-[var(--tint-indigo-fg)] animate-pulse" />
                      <span className="text-[12px] font-black uppercase tracking-[0.6em] text-[var(--tint-indigo-fg)] italic leading-none">Intelligence.Ledger.v9.4.2</span>
                   </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 relative z-10">
          {STATS.map((s, i) => (
            ...
            >
              <div className="absolute top-0 right-0 p-12 opacity-0 group-hover:opacity-5 transition-opacity duration-300"><Compass size={120} className="text-white" /></div>
              ...
              <div className="text-[14px] text-slate-400 font-black uppercase tracking-[0.4em] italic leading-none">{s.label}</div>
              <div className="text-[10px] text-slate-500 font-black uppercase tracking-widest mt-6 italic bg-white/5 px-6 py-2 rounded-full border border-white/5">{s.sub}</div>
            </motion.div>
          ))}
        </div>
        ...
        <div className={`${glassStyle} rounded-[6rem] overflow-hidden relative z-10 border-white/5 shadow-[0_60px_120px_rgba(0,0,0,0.5)]`}>
          <div className="px-20 py-16 border-b border-white/5 flex flex-col xl:flex-row items-center justify-between gap-12 bg-white/[0.02] relative overflow-hidden">
            ...
            <div className="flex items-center gap-10 relative z-10">
               <div className="w-18 h-18 bg-indigo-500/5 border border-[var(--tint-indigo-edge)] rounded-[2.2rem] flex items-center justify-center animate-pulse shadow-2xl"><Radio size={36} className="text-[var(--tint-indigo-fg)]" /></div>
               <div>
            ...
            <div className="px-12 py-5 rounded-[2.5rem] bg-indigo-500/5 border border-[var(--tint-indigo-edge)] text-[13px] font-black uppercase tracking-[0.5em] text-[var(--tint-indigo-fg)] shadow-[0_0_60px_rgba(99,102,241,0.1)] flex items-center gap-6 relative z-10">
            ...
            {platforms.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-16">
                {platforms.map(([platform, stats]) => {
                  ...
                  return (
                    <motion.div whileHover={{ y: -15 }} key={platform} className="bg-black/40 border border-white/5 rounded-[5rem] p-12 hover:border-[var(--tint-indigo-edge)] transition-all duration-700 group relative overflow-hidden shadow-2xl">
                      <div className="absolute inset-0 bg-gradient-to-br from-indigo-500/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                      <div className="flex items-center gap-8 mb-20 relative z-10">
                        <div className={`w-24 h-24 rounded-[3rem] bg-gradient-to-br ${cfg.gradient} flex items-center justify-center text-white font-black text-4xl shadow-2xl group-hover:scale-110 group-hover:rotate-12 transition-all duration-300 border border-white/10`}>{cfg.icon}</div>
                        <div>
                          <p className="text-[28px] font-black text-white uppercase italic tracking-tighter leading-none mb-3">{platform.toUpperCase()}</p>
                          <p className="text-[12px] text-slate-400 font-black uppercase tracking-[0.4em] italic leading-none">{stats.posts} NODES_ACTIVE</p>
                        </div>
                      </div>
                      <div className="space-y-16 relative z-10">
                        <div className="space-y-8">
                          <div className="flex justify-between items-center text-[11px] font-black uppercase tracking-[0.4em]">
                            <span className="text-slate-400 italic">Spectral_Saturation</span>
                            <span className="text-[var(--tint-indigo-fg)] tabular-nums italic bg-[var(--tint-indigo-bg)] px-4 py-1.5 rounded-xl border border-indigo-500/10">{fmt(stats.views)}</span>
                          </div>
                        ...
                        <div className="space-y-8">
                          <div className="flex justify-between items-center text-[11px] font-black uppercase tracking-[0.4em]">
                            <span className="text-slate-400 italic">Resonance_Signal</span>
                            <span className="text-[var(--tint-emerald-fg)] tabular-nums italic bg-[var(--tint-emerald-bg)] px-4 py-1.5 rounded-xl border border-emerald-500/10">{fmt(stats.engagement)}</span>
                          </div>
                          <div className="h-2.5 bg-black/60 rounded-full overflow-hidden shadow-inner border border-white/5">
        ...
        <div className={`${glassStyle} rounded-[6rem] overflow-hidden relative z-10 border-white/5 shadow-[0_60px_120px_rgba(0,0,0,0.5)]`}>
          <div className="px-20 py-16 border-b border-white/5 flex flex-col xl:flex-row items-center justify-between gap-12 bg-white/[0.02] relative overflow-hidden">
            <div className="flex items-center gap-10 relative z-10">
               <div className="w-18 h-18 bg-[var(--tint-amber-bg)] border border-[var(--tint-amber-edge)] rounded-[2.2rem] flex items-center justify-center animate-bounce shadow-2xl shadow-amber-500/20"><Award size={36} className="text-[var(--tint-amber-fg)]" /></div>
               <div>
            ...
            <div className="px-12 py-5 rounded-[2.5rem] bg-black/40 border border-white/5 text-[13px] font-black uppercase tracking-[0.5em] text-slate-400 italic relative z-10">
          ...
          <div className="divide-y divide-white/[0.03] bg-black/20">
            {data.top_performing_posts.map((post, idx) => (
              ...
              >
                <div className="absolute top-0 right-0 p-16 opacity-[0.01] group-hover:opacity-[0.05] transition-opacity duration-300"><Boxes size={300} /></div>
                <div className={`w-24 h-24 rounded-[3rem] flex items-center justify-center flex-shrink-0 text-white font-black text-5xl shadow-2xl relative z-10 transition-all duration-300 group-hover:-rotate-12 border border-white/20 ${
                ...
                <div className="flex-1 min-w-0 relative z-10">
                  <p className="text-[42px] font-black text-white italic uppercase tracking-tighter truncate group-hover:text-[var(--tint-indigo-fg)] transition-all duration-300 leading-none mb-6">{post.title?.toUpperCase() || 'UNNAMED_PHANTOM_NODE'}</p>
                  <div className="flex items-center gap-12">
                     <p className="text-[13px] text-slate-400 font-black uppercase tracking-[0.4em] italic flex items-center gap-5">
                        <Clock size={20} className="text-slate-500" /> {new Date(post.published_at).toLocaleDateString().toUpperCase()}
                     </p>
                ...
                <div className="flex items-center gap-24 flex-shrink-0 relative z-10">
                   <div className="text-center group/stat">
                      <p className="text-[12px] font-black text-slate-500 uppercase tracking-widest mb-4 italic group-hover/stat:text-[var(--tint-indigo-fg)] transition-colors">Saturation</p>
                   ...
                   <div className="text-center group/stat">
                      <p className="text-[12px] font-black text-slate-500 uppercase tracking-widest mb-4 italic group-hover/stat:text-[var(--tint-emerald-fg)] transition-colors">Resonance</p>
                      <p className="text-6xl font-black text-[var(--tint-emerald-fg)] italic tracking-tighter tabular-nums leading-none drop-shadow-2xl">{fmt(post.total_engagement)}</p>
                   </div>
                   <div className="text-center group/stat">
                      <p className="text-[12px] font-black text-slate-500 uppercase tracking-widest mb-4 italic group-hover/stat:text-purple-400 transition-colors">Affinity</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-16 relative z-10">
          {[
            { label: 'Spectral Log', desc: 'Cross-platform node historical records and terminal archives.', icon: Globe, href: '/dashboard/posts', gradient: 'from-blue-500/10 to-transparent', color: 'text-[var(--tint-indigo-fg)]' },
            { label: 'Flux Forecasting', desc: 'Deep resonance trajectory modeling and predictive analysis cluster.', icon: LineChart, href: '/dashboard/analytics/performance', gradient: 'from-emerald-500/10 to-transparent', color: 'text-[var(--tint-emerald-fg)]' },
            ...
            <motion.button 
              ...
              className={`${glassStyle} group flex flex-col gap-12 p-20 rounded-[6rem] transition-all duration-300 text-left relative overflow-hidden border-white/5 hover:border-indigo-500/50 shadow-[0_60px_120px_rgba(0,0,0,0.5)]`}
            >
              <div className="absolute top-0 right-0 p-16 opacity-0 group-hover:opacity-[0.05] transition-opacity duration-300"><Network size={180} className="text-white" /></div>
              <div className={`w-24 h-24 rounded-[3rem] flex items-center justify-center bg-white/[0.02] border border-white/10 flex-shrink-0 group-hover:scale-110 group-hover:rotate-12 transition-all duration-300 bg-gradient-to-br ${a.gradient} shadow-2xl`}>
              ...
              <div className="space-y-6">
                ...
                <p className="text-[14px] text-slate-400 font-black uppercase tracking-[0.4em] italic leading-relaxed">{a.desc}</p>
              </div>
              <div className="flex items-center gap-8 text-[16px] font-black text-[var(--tint-indigo-fg)] uppercase tracking-[0.6em] mt-auto group-hover:gap-10 transition-all duration-700 italic border-t border-white/5 pt-12">
