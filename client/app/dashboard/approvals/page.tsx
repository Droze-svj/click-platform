  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-48 bg-[var(--page-bg)] min-h-screen">
        ...
        <span className="text-[12px] font-black text-[var(--text-dim)] uppercase tracking-[0.6em] animate-pulse italic">{t('approvalsPage.loading')}</span>
      </div>
    )
  }
    ...
    <div className="min-h-screen relative z-10 pb-48 px-10 pt-16 max-w-[1700px] mx-auto space-y-24 font-inter">
      ...
      <header className="flex flex-col lg:flex-row items-center justify-between gap-12 relative z-50">
        <div className="flex items-center gap-10">
          <button type="button" onClick={() => router.push('/dashboard')} title={t('approvalsPage.abort')}
          ...
          <div>
            <div className="flex items-center gap-6 mb-4">
              <div className="flex items-center gap-3">
                ...
                <span className="text-[12px] font-black uppercase tracking-[0.8em] text-indigo-400 italic leading-none">{t('approvalsPage.workflowBadge')}</span>
              </div>
              <div className="px-6 py-2 rounded-full bg-black/60 border-2 border-white/5 shadow-inner">
                <span className="text-[10px] font-black text-emerald-400 tracking-widest uppercase italic leading-none">{t('approvalsPage.latticeShielded')}</span>
              </div>
            </div>
            <h1 className="text-5xl md:text-6xl font-black text-[var(--text-main)] tracking-tight leading-[1.05] drop-shadow-2xl">{t('approvalsPage.title')}</h1>
            <p className="text-[var(--text-dim)] text-sm md:text-base font-medium leading-relaxed max-w-2xl mt-3">{t('approvalsPage.subtitle')}</p>
          </div>
        </div>
        ...
        <button type="button" onClick={() => setShowInitializationModal(true)}
          ...
          <Plus size={32} className="group-hover:rotate-90 transition-transform duration-300" /> {t('approvalsPage.initializeConsensus')}
        </button>
      </header>
            ...
            >
                ...
                <div className="flex items-center justify-between mb-20 relative z-10 px-8">
                   <div className="flex items-center gap-10">
                      ...
                      <div>
                        <h2 className="text-6xl font-black text-[var(--text-main)] italic uppercase tracking-tighter leading-none mb-4 drop-shadow-2xl">{t('approvalsPage.pendingTitle')}</h2>
                        <p className="text-[14px] font-black text-[var(--text-dim)] uppercase tracking-[0.5em] italic leading-none border-l-4 border-amber-500/20 pl-8 ml-4">{t('approvalsPage.pendingSubtitle')}</p>
                      </div>
                   </div>
                   <div className="px-10 py-4 rounded-full bg-amber-500/10 border-2 border-amber-500/20 text-amber-500 text-[12px] font-black uppercase tracking-[0.3em] italic animate-bounce shadow-3xl">{t('approvalsPage.resonanceAlert', { count: pendingNodes.length })}</div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-12 relative z-10">
                  {pendingNodes.map((request) => (
                    ...
                    >
                         ...
                         <div className="space-y-6">
                            <div className="flex items-center gap-6">
                               <div className="px-8 py-2.5 rounded-full bg-indigo-500/10 border-2 border-indigo-500/20 shadow-3xl">
                                  <span className="text-indigo-400 text-[11px] font-black uppercase tracking-[0.4em] italic">{t('approvalsPage.nodeEntity', { entity: request.entityType.toUpperCase() })}</span>
                               </div>
                               <div className="flex items-center gap-3">
                                  <Clock size={14} className="text-[var(--text-dim)]" />
                                  <span className="text-[11px] font-black text-[var(--text-dim)] uppercase tracking-[0.3em] italic">{new Date(request.createdAt).toLocaleTimeString()}</span>
                               </div>
                            </div>
                            <div>
                               ...
                               <div className="flex items-center gap-4 py-2 px-6 rounded-2xl bg-black/40 border border-white/5 w-fit shadow-inner">
                                  ...
                                  <p className="text-[11px] font-black text-[var(--text-dim)] uppercase tracking-[0.3em] italic leading-none font-mono">{t('approvalsPage.idLabel', { id: request.entityId.substring(0, 16) })}</p>
                               </div>
                            </div>
                         </div>
                         <div className="flex flex-col items-end gap-4 text-right">
                            <span className="text-[10px] font-black text-amber-500/40 uppercase tracking-[0.5em] italic">{t('approvalsPage.resolutionFlux')}</span>
                      ...
                      {request.response && (
                        <div className="mb-12 p-8 rounded-[3rem] bg-indigo-500/5 border-2 border-white/5 italic text-[16px] font-black text-[var(--text-dim)] leading-relaxed uppercase tracking-tighter line-clamp-3 relative shadow-inner group-hover:border-indigo-500/20 transition-all duration-300">
                           <div className="absolute top-4 left-6 text-indigo-500 opacity-20"><MessageSquare size={24} /></div>
                           <span className="pl-12 block group-hover:text-white transition-colors duration-300">{t('approvalsPage.objectiveContext')}</span>
        ...
        <section className={`${glassStyle} p-24 rounded-[6rem] z-10 relative bg-black/40 border-white/5 shadow-[0_100px_300px_rgba(0,0,0,1)]`}>
            <div className="flex flex-col lg:flex-row items-center justify-between mb-20 px-8 gap-12 pt-4">
               <div className="flex items-center gap-10">
                  ...
                  <div>
                    <h2 className="text-5xl font-black text-[var(--text-main)] italic uppercase tracking-tighter leading-none mb-3 drop-shadow-2xl">{t('approvalsPage.historyTitle')}</h2>
                    <p className="text-[12px] font-black text-[var(--text-dim)] uppercase tracking-[0.5em] italic leading-none border-l-4 border-indigo-500/20 pl-8 ml-4">{t('approvalsPage.historySubtitle')}</p>
                  </div>
               </div>
               <div className="flex items-center gap-12">
                  <div className="px-10 py-4 rounded-full bg-black/60 border-2 border-white/5 shadow-inner">
                    <span className="text-[11px] font-black text-[var(--text-dim)] uppercase tracking-[0.6em] italic leading-none">{t('approvalsPage.protocolLogsAnalyzed', { count: matrix.length })}</span>
                  </div>
                  <div className="w-[2px] h-12 bg-white/10" />
                  <button className="w-16 h-16 rounded-[1.8rem] bg-white/[0.03] border-2 border-white/10 flex items-center justify-center text-[var(--text-dim)] hover:text-white hover:border-indigo-500/50 transition-all duration-700 shadow-3xl hover:scale-110 active:scale-90"><Search size={32} /></button>
               </div>
            </div>
            ...
            {matrix.length === 0 ? (
              <div className="py-48 text-center flex flex-col items-center justify-center gap-12 group">
                 ...
                 <div className="space-y-4">
                   <p className="text-3xl font-black text-white uppercase tracking-[0.8em] italic leading-none drop-shadow-2xl">{t('approvalsPage.emptyTitle')}</p>
                   <p className="text-[12px] font-black text-[var(--text-dim)] uppercase tracking-[0.4em] italic opacity-40">{t('approvalsPage.emptyState')}</p>
                 </div>
              </div>
            ) : (
                    ...
                    >
                      ...
                      <div className="flex-1 flex flex-col md:flex-row items-center gap-16 relative z-10 w-full">
                          <div className="w-24 h-24 rounded-[2.5rem] bg-black border-2 border-white/10 flex flex-col items-center justify-center gap-2 font-black shadow-inner flex-shrink-0 group-hover:border-indigo-400 transition-colors duration-300">
                             ...
                             <span className="text-[10px] text-[var(--text-dim)] uppercase leading-none tracking-widest">{new Date(request.createdAt).toLocaleString('default', { month: 'short' }).toUpperCase()}</span>
                          </div>
                          ...
                          <div className="flex-1 min-w-0 text-center md:text-left space-y-8">
                              <div className="flex flex-wrap items-center justify-center md:justify-start gap-8">
                                 ...
                                 <div className="flex items-center gap-4 py-1.5 px-6 rounded-full bg-white/[0.03] border border-white/5 italic">
                                   <span className="text-[10px] font-black text-[var(--text-dim)] uppercase tracking-widest">{t('approvalsPage.nodeEntity', { entity: request.entityType.toUpperCase() })}</span>
                                 </div>
                                 <div className="flex items-center gap-4 group/id cursor-pointer">
                                   ...
                                   <span className="text-[10px] font-black text-indigo-500 group-hover:text-white transition-all duration-300 uppercase tracking-widest italic font-mono">{t('approvalsPage.hashLabel', { hash: request.entityId.substring(0, 24) })}</span>
                                 </div>
                              </div>
                              
                              <div className="flex flex-col md:flex-row md:items-center gap-8 md:gap-16">
                                <div className="space-y-3">
                                   <p className="text-[11px] font-black text-[var(--text-dim)] uppercase tracking-[0.6em] italic opacity-40 leading-none">{t('approvalsPage.identMapPrimary')}</p>
                                ...
                                <div className="space-y-3">
                                   <p className="text-[11px] font-black text-[var(--text-dim)] uppercase tracking-[0.6em] italic opacity-40 leading-none">{t('approvalsPage.nodeRoleSigner')}</p>
                                   <p className="text-xl font-black text-indigo-400/60 transition-colors duration-300 group-hover:text-indigo-400 italic uppercase tracking-widest leading-none drop-shadow-2xl">
                                     {isInitiator ? t('approvalsPage.targetValidator') : t('approvalsPage.inboundInitiator')}
                                   </p>
                                </div>
                              </div>

                              {request.response && (
                                 <div className="p-8 rounded-[2.5rem] bg-black/60 border-2 border-white/5 italic text-[15px] font-black text-[var(--text-dim)] group-hover:text-white leading-relaxed uppercase tracking-tighter border-l-4 border-l-indigo-500/50 pl-10 transition-all duration-300 relative shadow-inner">
                                    <div className="absolute top-4 left-4 text-indigo-500 opacity-10"><Workflow size={24} /></div>
                                    <span className="opacity-30 block mb-2 text-[10px] tracking-[0.4em]">{t('approvalsPage.logResonanceData')}</span>
                                    "{request.response}"
                                 </div>
                              )}
                          </div>
                            ...
                            <div className="flex gap-6 lg:flex-col xl:flex-row">
                              ...
                              {isInitiator && (
                                <button type="button" onClick={() => handleAbortInit(request._id)}
                                  className="px-12 py-6 bg-white/5 border-2 border-white/10 text-[var(--text-dim)] hover:text-white hover:border-rose-500/50 rounded-[1.8rem] text-[12px] font-black uppercase tracking-[0.4em] transition-all duration-700 italic hover:bg-rose-600 active:scale-90 shadow-2xl">{t('approvalsPage.abortInit')}</button>
                              )}
                            </div>
                          )}
                          <button className="w-20 h-20 bg-white/5 border-2 border-white/10 rounded-[1.8rem] flex items-center justify-center text-[var(--text-dim)] hover:text-white transition-all duration-700 hover:bg-indigo-600 hover:border-indigo-400 hover:scale-110 active:scale-90 shadow-3xl group">
            ...
            >
              ...
              <div className="flex flex-col md:flex-row items-center justify-between mb-24 relative z-10 gap-12">
                <div className="flex items-center gap-12 text-center md:text-left">
                  ...
                  <div>
                     <h2 className="text-7xl font-black text-[var(--text-main)] italic uppercase tracking-tighter leading-none mb-4 drop-shadow-2xl">{t('approvalsPage.modalTitle')}</h2>
                     <p className="text-[13px] font-black text-[var(--text-dim)] uppercase tracking-[0.6em] italic leading-none border-l-4 border-indigo-500/20 pl-8 ml-4">{t('approvalsPage.modalSubtitle')}</p>
                  </div>
                </div>
                 ...
                 <div className="space-y-16">
                    <div className="space-y-10">
                       <div className="flex items-center gap-6 px-4">
                          ...
                          <label className="text-[14px] font-black text-[var(--text-dim)] uppercase tracking-[0.6em] italic">{t('approvalsPage.nodeEntitySelection')}</label>
                       </div>
                       <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                          {['content', 'script', 'payload'].map(type => (
                             <button type="button" key={type} onClick={() => setNewRequest({ ...newRequest, entityType: type })}
                               ...
                               {t('approvalsPage.nodeEntity', { entity: type.toUpperCase() })}
                             </button>
                          ))}
                       </div>
                    </div>
                    ...
                    <div className="space-y-10">
                       <div className="flex items-center gap-6 px-4">
                          ...
                          <label className="text-[14px] font-black text-[var(--text-dim)] uppercase tracking-[0.6em] italic">{t('approvalsPage.criticalityFlux')}</label>
                       </div>
                 ...
                 <div className="space-y-16">
                    <div className="space-y-10">
                       <div className="flex items-center gap-6 px-4">
                          ...
                          <label className="text-[14px] font-black text-[var(--text-dim)] uppercase tracking-[0.6em] italic">{t('approvalsPage.targetSignerIdentity')}</label>
                       </div>
                       <div className="relative group">
                          <input type="text" value={newRequest.requestedFrom} onChange={e => setNewRequest({ ...newRequest, requestedFrom: e.target.value })}
                            className="w-full px-12 py-10 bg-black/60 border-4 border-white/5 rounded-[3.5rem] text-[20px] font-black uppercase text-white tracking-[0.3em] focus:outline-none focus:border-indigo-500/50 transition-all duration-300 placeholder:text-[var(--text-dim)] italic shadow-inner group-hover:border-white/10"
                            placeholder={t('approvalsPage.placeholderOperatorId')}
                          />
                          <Key size={32} className="absolute right-12 top-1/2 -translate-y-1/2 text-[var(--text-dim)] group-hover:text-indigo-400 transition-colors duration-300 group-hover:rotate-12" />
                       </div>
                    </div>
                    ...
                    <div className="space-y-10">
                       <div className="flex items-center gap-6 px-4">
                          ...
                          <label className="text-[14px] font-black text-[var(--text-dim)] uppercase tracking-[0.6em] italic">{t('approvalsPage.payloadHashLabel')}</label>
                       </div>
                       <div className="relative group">
                          ...
                          />
                          <Anchor size={32} className="absolute right-12 top-1/2 -translate-y-1/2 text-[var(--text-dim)] group-hover:text-indigo-400 transition-colors duration-300 group-hover:-rotate-12" />
                       </div>
                    </div>
                    ...
                    <div className="space-y-10">
                       ...
                       >
                         ...
                         <Shield size={40} className="group-hover/start:rotate-12 transition-transform duration-700" /> {t('approvalsPage.startGateProtocol')}
                       </button>
                    </div>
                 </div>
              </div>
