...
export const SocialPublishingView: React.FC<SocialPublishingViewProps> = ({
  ...
  // Real per-platform optimal-time suggestions, derived from the user's own historical engagement.
  // Empty / null for a platform = not enough data yet → no suggestion shown (we never fabricate a time).
  const [optimalTimes, setOptimalTimes] = useState<Record<string, string | null>>({})

  useEffect(() => {
    let cancelled = false
    const tz = Intl.DateTimeFormat().resolvedOptions().timeZone
    apiGet<any>(`/scheduler/optimal-times?timezone=${encodeURIComponent(tz)}`)
      .then(res => {
        if (cancelled || !res?.success) return
        setOptimalTimes(res.data?.nextSuggested || {})
      })
      .catch(() => { /* silent: no recommendation is a valid state */ })
    return () => { cancelled = true }
  }, [])

  const formatSuggestion = (iso: string | null | undefined) => {
    if (!iso) return null
    try {
  ...
  const applyOptimalTime = (platformId: string) => {
    const iso = optimalTimes[platformId]
    if (!iso) return
    // Convert ISO → datetime-local string (YYYY-MM-DDTHH:mm) in the user's local zone
    const d = new Date(iso)
    const pad = (n: number) => String(n).padStart(2, '0')
    const local = `${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}T${pad(d.getHours())}:${pad(d.getMinutes())}`
    setPlatforms(prev => ({
      ...prev,
      [platformId]: { ...prev[platformId], scheduledAt: local, error: undefined, success: undefined }
    }))
    setScheduleOpenFor(platformId)
  }
                    ...
                    {optimalTimes[id] && (
                      ...
                      >
                        <span className="flex items-center gap-1.5 font-bold">
                          ...
                          Use peak time for your audience
                        </span>
                        <span className="font-mono">{formatSuggestion(optimalTimes[id])}</span>
                      </button>
                    )}
                    <input
                      id={`schedule-${id}`}
