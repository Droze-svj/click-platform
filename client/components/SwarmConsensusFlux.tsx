  ...
  if (loading && blocks.length === 0) {
    return (
      <div className={`${glassStyle} rounded-[3rem] p-12 min-h-[300px] flex items-center justify-center`}>
        <Cpu className="text-[var(--tint-indigo-fg)] animate-spin w-12 h-12 opacity-40" />
      </div>
    )
  }
  ...
  return (
    <div className={`${glassStyle} rounded-[3rem] p-10 relative overflow-hidden group`}>
      <div className="absolute top-0 right-0 p-8 opacity-[0.02] group-hover:opacity-[0.1] transition-opacity duration-[2s] pointer-events-none group-hover:rotate-45 group-hover:scale-125">
        <Workflow className="text-[var(--tint-indigo-fg)]" size={150} />
      </div>
      
      <div className="flex items-center justify-between mb-10 px-2">
         <div className="flex items-center gap-6">
            <div className="w-14 h-14 rounded-2xl bg-indigo-500/5 border border-[var(--tint-indigo-edge)] flex items-center justify-center shadow-lg group-hover:rotate-12 transition-all duration-700 animate-pulse">
               <Database className="text-[var(--tint-indigo-fg)]" size={28} />
            </div>
            <div>
               <h2 className="text-2xl font-black text-white italic uppercase tracking-tighter leading-none mb-1">Consensus Flux</h2>
               <p className="text-[10px] font-black text-slate-800 uppercase tracking-[0.4em] italic leading-none border-l-2 border-[var(--tint-indigo-edge)] pl-4 ml-1">Live Refinery Ledger // 2026.4_SOVEREIGN</p>
            </div>
         </div>
         <div className="flex items-center gap-3">
            <Signal size={12} className="text-[var(--tint-emerald-fg)] animate-ping" />
            <span className="text-[9px] font-black text-[var(--tint-emerald-fg)] uppercase tracking-widest italic">Lattice_Sync</span>
         </div>
      </div>
        ...
        <AnimatePresence mode="popLayout">
          {blocks.map((block) => (
            <motion.div 
              ...
              className={`p-6 rounded-[2rem] bg-black/40 border border-white/5 hover:border-[var(--tint-indigo-edge)] transition-all duration-700 group/item flex items-center justify-between shadow-xl relative overflow-hidden`}
            >
              <div className="flex items-center gap-6 relative z-10">
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center border border-white/10 shadow-inner ${
                  block.integrityScore > 90 ? 'bg-[var(--tint-emerald-bg)]' : 'bg-[var(--tint-amber-bg)]'
                }`}>
                  <Fingerprint size={20} className={block.integrityScore > 90 ? 'text-[var(--tint-emerald-fg)]' : 'text-[var(--tint-amber-fg)]'} />
                </div>
                <div>
                   <div className="flex items-center gap-3 mb-1">
                      <span className="text-[10px] font-black text-[var(--tint-indigo-fg)] uppercase tracking-widest italic">PASS_{block.refineryPass}</span>
              ...
              <div className="text-right relative z-10">
                 <div className="flex items-center justify-end gap-3 mb-1">
                    <BarChart3 size={12} className="text-[var(--tint-indigo-fg)] opacity-40" />
                 ...
                 <p className={`text-[8px] font-black uppercase tracking-widest italic leading-none ${
                   block.status === 'stable' ? 'text-[var(--tint-emerald-fg)]' : 'text-[var(--tint-amber-fg)]'
                 }`}>
      ...
      <div className="mt-8 pt-6 border-t border-white/5 flex items-center justify-between opacity-40 group-hover:opacity-100 transition-opacity">
         ...
         <button className="text-[9px] font-black text-[var(--tint-indigo-fg)] uppercase tracking-widest italic hover:underline">VIEW_FULL_LEDGER</button>
      </div>
    </div>
  )
}
