  ...
  return (
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
        <Card className="transition-shadow hover:shadow-lg">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              {t('predictiveAnalytics.performancePrediction')}
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger>
                  ...
                  <TooltipContent>
                    <p>{t('predictiveAnalytics.tooltipDescription')}</p>
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
            </CardTitle>
        ...
        <CardContent className="space-y-4">
          <div>
            <label className="text-sm font-medium mb-2 block">{t('predictiveAnalytics.title')}</label>
            <input
              type="text"
              value={contentData.title}
              onChange={(e) => setContentData({ ...contentData, title: e.target.value })}
              className="w-full p-2 border rounded"
              placeholder={t('predictiveAnalytics.contentTitlePlaceholder')}
            />
          </div>
          <div>
            <label className="text-sm font-medium mb-2 block">{t('predictiveAnalytics.body')}</label>
            <textarea
              value={contentData.body}
              onChange={(e) => setContentData({ ...contentData, body: e.target.value })}
              className="w-full p-2 border rounded"
              rows={4}
              placeholder={t('predictiveAnalytics.contentBodyPlaceholder')}
            />
          </div>
          <div>
            <label className="text-sm font-medium mb-2 block">{t('predictiveAnalytics.platform')}</label>
            <select
              value={contentData.platform}
              onChange={(e) => setContentData({ ...contentData, platform: e.target.value })}
              aria-label={t('predictiveAnalytics.platform')}
              title={t('predictiveAnalytics.platform')}
              className="w-full p-2 border rounded"
            >
          ...
          <Button onClick={predictPerformance} disabled={loading}>
            {loading ? (
              <>
                ...
                {t('predictiveAnalytics.predicting')}
              </>
            ) : (
              <>
                <TrendingUp className="mr-2 h-4 w-4" />
                {t('predictiveAnalytics.predictPerformance')}
              </>
            )}
          </Button>
            ...
            <div className="mt-4 space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-300">
              <div className="p-4 bg-gradient-to-r from-purple-50 to-blue-50 dark:from-purple-900/20 dark:to-blue-900/20 rounded-lg border border-purple-200 dark:border-purple-800">
                <div className="flex items-center justify-between mb-2">
                  <span className="font-medium">{t('predictiveAnalytics.performanceScore')}</span>
                  <Badge 
                ...
                <div className="grid grid-cols-2 gap-4 mt-4">
                  <div>
                    <p className="text-sm text-muted-foreground">{t('predictiveAnalytics.expectedViews')}</p>
                    <p className="font-semibold">
                      {prediction.expectedViews.min.toLocaleString()} - {prediction.expectedViews.max.toLocaleString()}
                    </p>
                  </div>
                  <div>
                    <p className="text-sm text-muted-foreground">{t('predictiveAnalytics.engagementRate')}</p>
              ...
              {prediction.positiveFactors.length > 0 && (
                <div>
                  <p className="text-sm font-medium mb-2">{t('predictiveAnalytics.positiveFactors')}</p>
              ...
              {prediction.recommendations.length > 0 && (
                <div>
                  <p className="text-sm font-medium mb-2">{t('predictiveAnalytics.recommendations')}</p>
                  <ul className="list-disc list-inside text-sm space-y-1">
