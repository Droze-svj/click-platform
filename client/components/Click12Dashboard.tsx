...
const ArbitragePanel = () => {
    ...
    const loadOffers = useCallback(async () => {
        try {
            ...
            showToast(t('click12Dashboard.failedToLoadOffers'), 'error')
        }
    }, [showToast])
    ...
    const steerFunnel = async (offerId: string) => {
        try {
            ...
            showToast(t('click12Dashboard.funnelSteered', { offerId }), 'success')
        } catch {
            showToast(t('click12Dashboard.steeringFailed'), 'error')
        }
    }
    ...
    return (
        <div className="space-y-12">
            <SectionHeader icon={DollarSign} title={t('click12Dashboard.arbitrageTitle')} subtitle={t('click12Dashboard.arbitrageSubtitle')} color="orange" badge={t('click12Dashboard.phase11Active')} />
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-10">
                      ...
                      className={`${glass} p-12 flex flex-col justify-between border-orange-500/10 hover:border-orange-500/30 transition-all duration-1000 relative overflow-hidden group min-h-[400px]`}>
                        ...
                        <div className="relative z-10">
                            <div className="flex items-center gap-4 mb-4">
                              <span className="text-[10px] text-orange-400 uppercase font-black tracking-[0.4em] italic px-5 py-2 rounded-full bg-orange-500/5 border border-orange-500/20">{t('click12Dashboard.velocityFlux', { velocity: offer.velocity })}</span>
                            ...
                            <div className="flex gap-12 mb-12">
                                <div className="space-y-2">
                                    <p className="text-[10px] font-black text-slate-800 uppercase tracking-widest italic opacity-40">{t('click12Dashboard.conversionResistance')}</p>
                                ...
                                <div className="space-y-2 border-l-2 border-white/10 pl-12">
                                    <p className="text-[10px] font-black text-slate-800 uppercase tracking-widest italic opacity-40">{t('click12Dashboard.payoutCapitalValue')}</p>
                ...
                {offers.length === 0 && (
                    <div className={`${glass} p-24 text-center col-span-2 flex flex-col items-center justify-center space-y-8 border-dashed border-white/10`}>
                        ...
                        <p className="text-2xl font-black text-slate-800 uppercase tracking-widest italic opacity-40">{t('click12Dashboard.noArbitrageVectors')}</p>
                    </div>
                )}
            </div>
        </div>
...
const EncirclementPanel = () => {
    ...
    const loadNetwork = useCallback(async () => {
        try {
            ...
            showToast(t('click12Dashboard.s2sNetworkDown'), 'error')
        }
    }, [showToast])
    ...
    const stats = [
        { label: t('click12Dashboard.latticeHealth'), value: typeof network?.health === 'number' ? (network.health * 100).toFixed(1) : '---', unit: '%', color: 'text-purple-400', icon: ShieldCheck },
        { label: t('click12Dashboard.aggregatedRevenue'), value: typeof network?.overLordStats?.totalRevenueAggregated === 'number' ? (network.overLordStats.totalRevenueAggregated / 1000000).toFixed(1) : '---', unit: 'M+', color: 'text-emerald-400', icon: TrendingUp },
        { label: t('click12Dashboard.victoryVectors'), value: network?.overLordStats?.victoriesToday ?? '---', unit: 'FLUX', color: 'text-fuchsia-400', icon: Zap },
    ]

    return (
        <div className="space-y-16">
            <SectionHeader icon={Terminal} title={t('click12Dashboard.encirclementTitle')} subtitle={t('click12Dashboard.encirclementSubtitle')} color="purple" badge={t('click12Dashboard.godModeSaturationActive')} />
            ...
            <div className={`${glass} p-12 space-y-8 border-purple-500/20 relative overflow-hidden bg-black/40 shadow-inner group hover:border-purple-500/40 transition-all duration-1000`}>
                <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-4">
                        ...
                        <span className="text-[11px] font-black text-purple-400 uppercase tracking-[0.6em] italic leading-none">{t('click12Dashboard.decentralizedIntelligenceLedger')}</span>
                    </div>
                </div>
                <div className="space-y-4 font-mono text-[13px] text-purple-300 leading-relaxed max-h-[300px] overflow-y-auto pr-8 custom-scrollbar">
                    {ledger.length === 0 ? (
                        <p className="opacity-40 uppercase tracking-widest border-b border-purple-500/10 pb-2 mb-4">{t('click12Dashboard.awaitingLedgerPulse')}</p>
                    ) : (
                        ledger.map((entry: any, i: number) => (
                            <p key={i} className="flex gap-4"><span>{entry.timestamp ?? '--:--:--'}</span> <span className="text-white font-bold">{entry.channel ?? t('click12Dashboard.ledgerSystemChannel')}</span> <span>{entry.message ?? ''}</span></p>
                        ))
                    )}
                </div>
            </div>
...
const StabilityPanel = () => {
    ...
    const loadHealth = useCallback(async () => {
        try {
            ...
            setSubsystems([])
            showToast(t('click12Dashboard.systemHealthUnavailable'), 'error')
        }
    }, [showToast])
    ...
    return (
        <div className="space-y-16">
            <SectionHeader icon={Shield} title={t('click12Dashboard.stabilityTitle')} subtitle={t('click12Dashboard.stabilitySubtitle')} color="cyan" badge={t('click12Dashboard.liveBridgeStatus')} />
            ...
            {subsystems && subsystems.length > 0 ? (
                ...
                <div className={`${glass} p-24 text-center flex flex-col items-center justify-center space-y-8 border-dashed border-cyan-500/10`}>
                    ...
                    <p className="text-2xl font-black text-slate-800 uppercase tracking-widest italic opacity-40">
                        {subsystems === null ? t('click12Dashboard.loadingBridgeStatus') : t('click12Dashboard.noConnectedBridges')}
                    </p>
                </div>
            )}
            ...
            <div className={`p-10 rounded-[3rem] ${glass} bg-rose-500/5 border-rose-500/20 flex flex-col md:flex-row items-center justify-between gap-12 hover:border-rose-500/40 transition-all duration-700 shadow-[0_50px_150px_rgba(244,63,94,0.1)]`}>
                <div className="flex items-center gap-8">
                    ...
                    <div>
                        <h4 className="text-3xl font-black text-[var(--text-main)] italic uppercase tracking-tighter leading-none mb-2">{t('click12Dashboard.emergencyNodeKillswitch')}</h4>
                        <p className="text-[12px] font-bold text-slate-800 uppercase tracking-[0.4em] italic opacity-60">{t('click12Dashboard.killswitchSubtitle')}</p>
                    </div>
                </div>
            </div>
        </div>
...
export default function Click12Dashboard() {
  ...
  const TABS = [
    { id: 'resonance', label: t('click12Dashboard.tabResonance'), icon: Brain, color: 'indigo' },
    { id: 'arbitrage', label: t('click12Dashboard.tabArbitrage'), icon: DollarSign, color: 'orange' },
    { id: 'encirclement', label: t('click12Dashboard.tabEncirclement'), icon: Globe, color: 'purple' },
    { id: 'stability', label: t('click12Dashboard.tabStability'), icon: Shield, color: 'cyan' }
  ]
              ...
              <div>
                 <div className="flex flex-col md:flex-row items-center gap-6 mb-4">
                    <div className="flex items-center gap-3">
                       ...
                       <span className="text-[12px] font-black uppercase tracking-[0.5em] text-purple-400 italic leading-none">{t('click12Dashboard.versionLabel')}</span>
                    </div>
                    <div className="flex items-center gap-3 px-5 py-2 rounded-full bg-black/60 border border-white/5 shadow-inner">
                        ...
                        <span className="text-[10px] font-black text-slate-800 tracking-widest uppercase italic leading-none">{t('click12Dashboard.overlordTerminalSynced')}</span>
                    </div>
                 </div>
                 <h1 className="text-8xl font-black text-[var(--text-main)] italic uppercase tracking-tighter leading-none mb-4 drop-shadow-[0_0_30px_rgba(255,255,255,0.2)]">
                   {t('click12Dashboard.mainTitle')}
                 </h1>
                 <p className="text-slate-800 text-[16px] uppercase font-black tracking-[0.5em] italic leading-none opacity-80 border-l-4 border-purple-500/40 pl-8 ml-2">{t('click12Dashboard.mainSubtitle')}</p>
              </div>
           </div>
