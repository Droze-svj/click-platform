...
class YouTubeOAuthService {
  ...
  async exchangeCodeForToken(userId, code, state) {
    ...
    if (!user || !user.oauth?.youtube?.state || user.oauth.youtube.state !== state) {
      logger.warn('YouTube OAuth exchange: state mismatch', { ...LOG_CONTEXT, userId });
    ...
    return { success: true, platform: 'youtube', profile };
  }
  ...
  async disconnectAccount(userId) {
    await User.findByIdAndUpdate(userId, {
      $set: { 'oauth.youtube': { connected: false } }
    });
