...
async function getConnectedAccounts(userId) {
  const user = await User.findById(userId).select('oauth.facebook');
  if (!user || !user.oauth?.facebook?.connected) return null;
  return {
    platform: 'facebook',
    username: user.oauth.facebook.platformUsername,
    avatar: user.oauth.facebook.avatar, // Assuming it might be stored
    connectedAt: user.oauth.facebook.connectedAt
  };
}

async function refreshAccessToken(userId) {
  logger.info('Facebook token refresh not yet implemented (Facebook tokens are typically long-lived)', { userId });
  return null;
}
