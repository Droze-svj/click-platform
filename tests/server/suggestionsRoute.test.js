...
describe('/api/suggestions/daily — real personalized engine', () => {
  ...
  it('shapes REAL ideas + a content-gap nudge (the old hardcoded array is gone)', async () => {
    ...
    const res = await request(app).get('/api/suggestions/daily').set('x-uid', '6a3500000000000000000aaa');
  ...
  it('honest cold-start for dev users (empty, no AI call)', async () => {
    const res = await request(app).get('/api/suggestions/daily').set('x-uid', 'dev-user-123');
  ...
  it('/daily-ideas + /content-gaps proxy the real service', async () => {
    ...
    const a = await request(app).get('/api/suggestions/daily-ideas').set('x-uid', '6a3500000000000000000bbb');
    expect(a.body.data[0].title).toBe('idea A');
    const g = await request(app).get('/api/suggestions/content-gaps').set('x-uid', '6a3500000000000000000bbb');
